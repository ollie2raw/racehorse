...
**As of 2026-09-05: Systems 1–13 — the plan's entire originally-numbered sequence — are all closed for their audited scope.** This section used to carry a running per-system narrative; that duplicated each system's own §X sections below and was the reason it went stale (last accurate as of System 7). It's now a short index instead: one line per system's closure state, then the actual remaining work — named parked pools, not a queue of open systems.

**Closure summary (detail lives in each system's own §X section and the decisions log, not repeated here):**

| System | State | Ratified |
|---|---|---|
| Cross-cutting security sweep (before §1) | Closed | — |
| 1 — Tournament | Closed | D-3 |
| 2 — Multiplayer rooms | Closed (Tiers A–E) | D-9 |
| 3 — Daily modes | Closed | D-10 |
| 5 — Legacy League | Closed (decommissioned; confirmed fully absent from the live schema by System 13) | — |
| 6 — Auth / session / rate limiting | Closed | D-13 |
| 7 — `@racehorse/game-core` | Closed | D-14 (+ GC-5 same-day incident fix) |
| 8 — Ranking / Glicko-2 | Closed **for audited scope only** — RK-5 closed 2026-09-05 (shared `supabaseFetch`); RK-6 closed 2026-09-08 (boot + periodic catch-up sweep); RK-3 ratified REVISIT IF SCALE, still untouched | D-15 |
| 9 — Match runtime layer | Closed **for audited scope only** — 5 of §9.1.12's original 9 items formally parked, not covered | D-16, D-18 |
| 10 — Individual game modes | Closed | D-19 |
| 11 — Social / stats / account | Closed | D-20 |
| 12 — Progression & learning | Closed | D-21 |
| 13 — Remaining cross-cutting / infra | Closed | D-22 |

**The actual remaining work — parked pools, explicitly deferred, not lost:**

1. **System 9's 5 items** (D-18): `modules/guided/` (18 files), `modules/daily-puzzle/` (2 files, likely dead), `client/src/match/board/` (rendering), `useLiveMatchSession.ts`'s composed hooks, the review hooks (`usePostGamePivotalReview.ts`/`useReviewRuntime.ts`). Deferred to a dedicated future pass — System 9 is not "exhaustively audited" the way Systems 1–8/10–13 are until these are explicitly re-opened.
2. **System 11's SA-4 and SA-7 — both CLOSED.** **SA-7 CLOSED 2026-09-05** — the friend-request race (two users requesting each other simultaneously both pass the existing-pair read, both INSERT, the loser hits `friends_pair_unique_idx` and PostgREST's `23505` 409 was turned into a generic `500` by the route catch). Confirmed the race is on the **request/send** path, exactly as §11.3 describes — not concurrent *accepts* (those PATCH the same `status='accepted'`, idempotent, no conflict). Both `/friends/request*` routes now catch the pair-unique conflict (`friends_pair_unique_idx|23505`, matching `puzzleRushStore.ts`'s pattern) and return `409 "A friend request between you already exists."`; a genuine non-conflict error still surfaces as `500`. DB invariant SA-INV-5 was already holding — this only fixes the status code. New `socialFriends.test.ts` (4 cases). **SA-4 CLOSED 2026-09-08 (D-27)** — no lock guarded `/api/ghost/complete` against a genuinely concurrent double-completion; ratified REVISIT IF SCALE at D-20, closed once wider traffic made that deferral's own trigger condition live. `withGhostCompletionLock` (mirroring Daily Fritz's `withDailyFritzAttemptLock`) now guards the route end-to-end; the pre-fix race was confirmed with a failing repro before fixing it, not just asserted. §11.3's SA-4 row carries the full investigation.
3. **System 13's CC-3** (D-22): `daily_puzzles`' admin RLS gates on a hardcoded email instead of this app's `ADMIN_SECRET` pattern — POSTURE, worth doing opportunistically next time that file is touched, not worth a dedicated pass. **CC-5 CLOSED 2026-09-05** — `sentryScrubbers.ts`'s `isSensitiveKey` extended to `password` / `refresh_token` / `refreshToken` / `email` (kept exact-match, consistent with the existing mechanism, so adjacent fields like `email_confirmed` are untouched). Confirmed genuinely risk-free posture, not a live behaviour change: **zero** references to any of these fields anywhere in non-test `server/src` — auth is entirely client-side against Supabase, the server never handles credentials. Test added to the existing `sentryScrubbers.test.ts`.

**A fourth pool, found while writing this refresh, not one of the three named when this refresh was requested — surfaced rather than silently left out:** **System 8's RK-3** (D-15, §8.3), ratified **REVISIT IF SCALE** and still untouched — client-side rating-prediction math omits the server's forfeit-outcome override; cosmetic-adjacent, mispredicts a number nobody currently relies on. Not urgent. **RK-6 CLOSED 2026-09-08 (D-25)** — the weekly rating-period cron's missing boot-time catch-up sweep is fixed: `startRankingCron()` now also sweeps `processAllPendingRatingGames()` at boot and every 15 minutes, matching the pattern this codebase already uses for its other reapers; the mid-sweep-restart atomicity question this row used to flag as unconfirmed is resolved (safe by construction — `commit_glicko_game_update` is one transaction per game). **RK-5 CLOSED 2026-09-05** — `ghost/service.ts` now imports `supabaseUtils.ts`'s shared `supabaseFetch` instead of its local copy. Confirmed functionally equivalent for ghost's usage before deleting (all reads are SELECTs that return `[]`, all writes use `return=representation`; the local `[]`-on-204 vs shared `undefined`-on-empty-body difference is unreachable); the shared helper additionally brings ghost in line with the rest of the server on pooler-URL routing and Sentry spans/slow-query logging that RK-5 flagged as the drift risk. **Not purely behaviour-neutral, deliberately:** the shared helper applies a **15s request timeout** the local copy never had, so a ghost call to a hung PostgREST endpoint now throws `Supabase request timed out after 15000ms` instead of blocking indefinitely — a favourable trade (a bounded failure is better than an unbounded one, and this is what every other server DB call already does), recorded here so a future "why did ghost start throwing timeout errors" question lands on this line, not a re-derivation. Full `ghost/` suite (65 tests) + transitive callers green, zero test changes.

**A fifth pool, added 2026-09-05 for findability:** **`ranked_games.opponent_id`** — an orphaned bare-uuid column (no longer a live FK to `profiles`) that Guardrail #6's cascade-delete check cannot see by design, described in `ENGINEERING_GUARDRAILS.md` §6's "Known limitation" note (and already flagged in `account/routes.ts`'s own deletion note). Non-blocking posture concern: a stored id that no longer resolves to a profile, not a deletion-blocking FK. Not scheduled; recorded here so it's tracked the way the other four pools are.

**A sixth pool, added 2026-09-05, rewritten once Finding B landed:** **the entire Fritz-Challenge server code path is unmounted dead code on `main`.** `registerFritzChallengeRoutes` is never called on `main` — the `/api/fritz-challenges/*` endpoints do not exist on the running server (they 404); the client UI and route wiring live only on `feat/fritz-challenge-multi`, last touched 2026-08-02, never merged (see "Finding 2026-09-05: FC-DEAD-1" below and the §10.1.3 correction). The Guardrail #4 investigation's finding — `verifyDailyFritzHand` called without `requireStateDigests` at `fritzChallenges.ts:407` / `:515`, an RT-2-shape copy-paste asymmetry — is **real but moot**: there is no code path to reach it while the routes are unmounted. It becomes relevant only if the feature is revived from its branch, at which point the fix (`requireStateDigests: true` at those two sites, or port the authority-contract pinning into a mounted `/start`) is a revival task, not a hardening task. Not independently exploitable even if activated as-is (the server's engine-legality + Fritz-policy-parity reconstruction do not trust client state; the digest is defence-in-depth). See `ENGINEERING_GUARDRAILS.md` §4's `verifyDailyFritzHand` scoping paragraph. **FC-DEAD-1 CLOSED 2026-09-10** — the ~2.3k LOC deleted whole (`fritzChallenge.ts`, `http/routes/fritzChallenges.ts` + its 2 test files, `http/stores/fritzChallengeStore.ts`/`fritzChallengeCommandStore.ts` + their test files), plus the orphaned `soak:fritz-challenge-authority` npm script (already pointing at a missing `scripts/fritzChallengeAuthoritySoak.ts`). Verified zero non-test importers of the whole cluster before deleting (nothing outside it referenced `fritzChallenge`/`fritzChallenges`/`fritzChallengeStore`/`fritzChallengeCommandStore`); the `requireStateDigests` residual this paragraph flags as moot is gone with it. `tsc -p tsconfig.json`, `typecheck:scripts`, `build`, and the full server suite (220/220 files, 1327/1327 tests, down from 224/1342 — exactly the 4 deleted test files) all green. The `fritz_challenge*` tables/migrations and their RLS policies are untouched — `check:policy-manifest` / `check:cascade-delete` still see them live, deliberately; dropping them is a separate migration, not bundled here.

**Two guardrails came directly out of this session's live findings**, beyond the pools above (`ENGINEERING_GUARDRAILS.md`): Guardrail #1 (RLS/policy assertions) was extended from 4 to 5 pinned tables the same day System 13 found and fixed the `matches` forgery; Guardrail #6 (account-deletion cascade completeness) was newly written after System 11's `bot_match_pending` finding and then **built to completion 2026-09-06 (D-24)** — its first prod run immediately found **AD-1**, a live account-deletion bug (`ghost_games` / `ghost_profiles` / `matches` FKs that weren't what every reference file claimed; own dated section below). **Guardrails #1, #2, #3, #4, #6, and #7 are now built and enforcing in CI** (INV-16/17/18/19 in `check:architecture`, plus Guardrail #6's cascade check and Guardrail #7's freshness detector in `security-posture.yml`); **#5 (staging/canary before 100% prod) has a ratified direction (D-26, 2026-09-08) rather than an unbuilt mechanism** — the scoping doc's own recommendation was adopted as-is: no canary environment at this scale (priced, and checked against AD-1/SA-6/DF-STALE-1 — it would not have caught any of them), Vercel preview deployments as the pre-merge habit, and the already-shipped light feature-flag manifest (`docs/server-feature-flags.md`) as the staged-enable safety valve (see `ENGINEERING_GUARDRAILS.md` §5).

**One incident after closure (2026-09-05):** a Daily Fritz `/start` outage (**DF-STALE-1**, D-23) — a day-ahead pre-generated published challenge stranded by an unrelated mid-window `FRITZ_POLICY_VERSION` bump. Fixed same day, code-only (`f37cf0ef`), verified live. It surfaced a **genuinely new bug class** none of Systems 1–13 covered — temporal staleness of a version-stamped published artifact — written up as its own dated section (`# Incident 2026-09-05: …`, just before the Appendix). **Guardrail #7 is now BUILT (both halves), 2026-09-05:** reuse-first serving pinned by INV-19 (`check:architecture`) across all three publish call sites, plus a proactive freshness detector (`checkPublishedArtifactFreshness.ts` in `security-posture.yml`) that runs the serving path's own validity oracle against future-dated rows — verified against prod. `ENGINEERING_GUARDRAILS.md` §7.

**Picking this up cold:** there is no "next system" to start — the numbered sequence is done. **Guardrails #2, #3, #4, #6, #7 are now BUILT** (INV-16/17/18/19 in `check:architecture`, Guardrail #6's cascade check, Guardrail #7's freshness detector); **#1 was built earlier**; **#5 (staging/canary) has a ratified direction (D-26) — no canary environment, Vercel-preview habit + the already-shipped light flag manifest instead** — the one open thread on it is a human confirming Vercel preview deployments are actually on and fixing the noted `.vercel/project.json` mislink, not a build. Decide among: (a) one of the six parked pools above, (b) the two small Guardrail #5 follow-ups just named, (c) something outside this plan's original scope entirely. None carry the urgency the numbered systems did — treat this file as complete for its stated purpose (Systems 1–13 hardened) unless new live evidence says otherwise.
...
`insertRankedGameIdempotent()` does the correct thing: when the caller supplies
a non-empty `source.sourceMatchId`, it POSTs with
`?on_conflict=player_id,source_match_id` +
`Prefer: resolution=ignore-duplicates` — a real dedup guard. Absent a
`sourceMatchId`, it falls through to a plain POST with **zero** dedup
(`hasIdempotentSource()` gate). *(As originally found this was ALSO gated on
`RANKED_GAMES_SOURCE_COLUMNS_ENABLED=true`; that flag was the RK-8 gap and was
**deleted 2026-09-06** — the source-column write + `on_conflict` are now
unconditional. See RK-8 in §8.3.)*
...
(untouched):** RK-3, RK-6 (RK-5 closed 2026-09-05 — shared `supabaseFetch`).
**RK-7** (the rematch/`bot_match_pending`
...
| **RK-5** | **`ghost/service.ts` maintains its own local `supabaseFetch`, duplicating `supabaseUtils.ts`'s shared helper.** Same request shape (`apikey` + `Authorization: Bearer`, `SUPABASE_SERVICE_KEY`), independently wired, confirmed while verifying RK-0's fix didn't break this path. No behavioral gap today (verified via RK-0's investigation that it authenticates identically as `service_role`) — this is a code-hygiene / single-source-of-truth finding, not a security or correctness one. | §8.1.3 (RK-0 verification) | **process** | n/a — cosmetic/maintenance, not risk-scored | none today; a future divergence (e.g. a timeout or circuit-breaker change made to `supabaseUtils.ts`'s version but not mirrored here) would silently apply only to non-ghost writers | **CLOSED 2026-09-05** — `ghost/service.ts` now imports the shared `supabaseFetch`; its local copy (35 lines: `supabaseFetch` + `getConfig`/`requireEnv`) deleted. Equivalence verified before the swap: every ghost call site reads a SELECT (returns `[]` JSON, never a 204) or writes with `return=representation` (returns the row array), so the local `[]`-on-204 vs shared `undefined`-on-empty-body divergence is unreachable; the shared version additionally applies `SUPABASE_POOLER_URL` routing and Sentry span / slow-query logging that the local copy never received — i.e. drift this row predicted had already accumulated on the shared side. **One deliberate, non-neutral change:** the shared helper enforces a **15s request timeout** that ghost's local copy did not — a ghost call to a hung PostgREST endpoint now throws `Supabase request timed out after 15000ms` rather than blocking indefinitely. Favourable trade (bounded > unbounded failure; every other server DB call already does this), noted so a future ghost-timeout investigation finds the cause here. Full `ghost/` suite (10 files / 65 tests) + `gameOverPersistence` / `ghostMoveLogCapture` / `mpInvariantHarness` green with **zero test changes**. | — |
| **RK-6** | **The deferred rating-period cron has no boot-time catch-up sweep and an unconfirmed restart story.** `startRankingCron()` runs weekly (Sundays 00:00 UTC) via in-process `node-cron`; unlike other reapers in this codebase (boot sweep + frequent interval), nothing re-triggers `processAllPendingRatingGames()` between scheduled runs, and the sweep loop's atomicity across rows on a mid-sweep process restart was not confirmed this session. | §8.1.2 | **availability** | was **low-medium** — Render free tier restarts are not rare; any row inserted between Sundays that missed the inline path sat `rating_after: null` (unranked-looking) for up to 7 days | any ranked game whose inline processing failed, until the next Sunday | **CLOSED 2026-09-08 (D-25).** Mid-sweep-restart question resolved first, per instruction: `commit_glicko_game_update` commits one game's rating update in a single Postgres transaction (one plpgsql function call), and every sweep re-queries `rating_after=is.null` fresh — so a restart mid-sweep already left no partial state; the fix needed no new locking, only the missing trigger cadence. `startRankingCron()` now also fires a boot sweep (20s after boot) and a periodic sweep (every 15 min) of `processAllPendingRatingGames()`, mirroring `dailyFritzStrandedRecovery.ts`'s reaper shape. New `cron.test.ts` (6 cases, fake timers) proves a mid-week pending game is processed on the next periodic tick, not the following Sunday. Full server vitest (219/1306) + server build green. | RK-INV-4 |
| **RK-8** | **`insertRankedGameIdempotent()`'s dedup is entirely conditional on `RANKED_GAMES_SOURCE_COLUMNS_ENABLED === 'true'`, and that flag has no floor — no `getEnvBool` default, no boot assertion, not recorded anywhere as load-bearing.** Found 2026-09-05 while scoping Guardrail #5 Option B (`docs/server-feature-flags.md`). **Trace (from the code, not the name):** `hasIdempotentSource()` (`insertRankedGameIdempotent.ts:27`) returns `isRankedGameSourceColumnsEnabled() && Boolean(sourceMatchId)`. Flag false/unset → `false` → the `if (!hasIdempotentSource(input))` branch (`:41`) runs a **plain unconditional `POST /rest/v1/ranked_games`** — no `on_conflict`, no `Prefer: resolution=ignore-duplicates`. The dedup guard is *dropped entirely*, not degraded. Three compounding reasons nothing else catches it: (1) `buildRankedGameInsertPayload()` gates the `source_type`/`source_match_id` write on the **same** flag (`rankedGamePayload.ts:85`), so the row goes in with `source_match_id = NULL`; (2) the DB unique index `ranked_games_player_source_match_uidx ON (player_id, source_match_id)` is a **full** index whose own migration comment (`2026-06-18_ranked_games_source_conflict_target.sql`) states *"NULL source ids remain non-conflicting under PostgreSQL's default NULL semantics"* — so the index does not reject the duplicate; (3) the callers' own skip-duplicate check (`if (!insertResult.isNew)` in `fritzMatchLifecycle.ts` / `ghost/service.ts`) never fires, because a plain POST with `return=representation` always returns the row → `isNew: true` → the Glicko update is applied a **second** time. **This directly defeats Guardrail #3 / INV-17 (`ENGINEERING_GUARDRAILS.md` §3).** INV-17 statically verifies every `ranked_games` POST routes through the wrapper — but the wrapper's idempotency is *itself* runtime-flag-gated, so INV-17 green ≠ "ranked writes are idempotent." RK-1/RK-2's shipped fixes (route the two Fritz-branch inserts through the wrapper) inherit the same conditionality. **Live state confirmed directly 2026-09-05** (service-role query, not §8.1.3's older sample): of 476 prod `ranked_games` rows, the 118 most recent — every row since ~2026-08-01 — carry a non-NULL `source_match_id`; **0 duplicate `(player_id, source_match_id)` keys** in that window; the 358 NULL-`source_match_id` rows are all historical (newest `2026-08-01`, pre-flag). So the flag is effectively ON in the running prod service and the guard is functioning **today**. | §8.1.3 (extends RK-1/RK-2) | **integrity-oracle** (a rating double-applied) — *conditional on the flag being unset, which is not the current state* | **low today** (flag confirmed set; guard confirmed working). **Elevated by:** (a) env-var cleanup / mistaken removal — nothing stops it; (b) **any new server environment** — a canary / shadow / staging service (exactly what the Guardrail #5 scoping doc weighs) stood up without this env var silently has non-idempotent ranked writes from row one; (c) a prod re-provision from config | every ranked game inserted while the flag is off, across all four source types (`live_room`, `verified_single_player`, `local_fritz_abandon`, `scheduled_tournament`), via the retry-loop and re-triggered-sweep re-invocation vectors RK-1/RK-2 identified | **FIX NOW — stopgap SHIPPED 2026-09-05 (human override of the POSTURE grade: reachable today by an ordinary env-var/redeploy mistake, not gated behind unreachable code like F21/F22; fix is a strictly-safer default with zero change to the current correct state).** **Done:** `isRankedGameSourceColumnsEnabled()` now reads `getEnvBool('RANKED_GAMES_SOURCE_COLUMNS_ENABLED', true)` — default intentionally inverted from the codebase's usual `false` (comment states why + points here); `getEnvBool` exported from `config.ts` so the helper doesn't reimplement `=== 'true'`. Explicit `=false`/`=0` still forces the legacy plain-insert path — the escape hatch stays, only its default is fixed. `index.ts` startup block logs `log.error` + `Sentry.captureException` (prod) if the flag ever resolves false, GC-1-style (loud, not a throw — a deliberate `=false` is legitimate). Tests: unset → resolves `true` + payload carries source columns + wrapper takes the `on_conflict` path; explicit `=false` → legacy path still works. `isRankedGameOutcomeColumnEnabled()` **left as-is** — its column exists in prod but the migration is recent (`2026-08-28`) and 0 prod rows carry a non-NULL `outcome` (ambiguous: flag-off vs no-ranked-forfeit-since), so a default-flip there would be a real behaviour change, not a no-op. **CLEAN FIX SHIPPED 2026-09-06 — RK-8 now fully closed.** `RANKED_GAMES_SOURCE_COLUMNS_ENABLED` is **deleted**: the `source_type`/`source_match_id` write (`buildRankedGameInsertPayload`) and the `on_conflict=player_id,source_match_id` path (`insertRankedGameIdempotent`) are unconditional; the flag helper, the `index.ts` boot `log.error`/Sentry check, `getEnvBool`'s now-unused `export`, and all four other consumer references (`gameOverPersistence.ts` log field, `fritzMatchLifecycle.ts` `resolveLocalFritzAbandonRankedSource` early-return, `privateMatchResult.ts` `queryRankedGameForMatch` early-return) are gone. `hasIdempotentSource()` is now just `Boolean(sourceMatchId)` — the sole surviving conditional is the genuine no-match-identity path (`completeGhostGame` with a null `matchId` → `source: null` → plain POST), which the **full** unique index treats as non-conflicting by design (traced: none of the four source types can reach the insert with a null id via any real call path; `completeGhostGame`'s `matchId?: string \| null` signature is the only door and no caller passes null today). Live re-confirm 2026-09-06 (service-role): 476 prod rows; **every row since ~2026-08-10 carries `source_match_id`** (most recent NULL is `2026-08-01`); **0 duplicate `(player_id, source_match_id)` keys**; the 358 NULL rows are all pre-migration history. Tests repurposed as reintroduction regression guards: `RANKED_GAMES_SOURCE_COLUMNS_ENABLED=false` now asserted to have **no effect** on both the payload and the wrapper path. No `.env.example` line existed. Verified: server `tsc -b` + full vitest, client `tsc -b` / lint / `check:architecture` / vitest.<br><br>**CORRECTION 2026-09-07 — the "Live re-confirm 2026-09-06" conclusion above is RETRACTED.** That service-role query counted *all* 476 `ranked_games` rows and never filtered on `source_type`. "Every row since ~2026-08-10 carries `source_match_id`, 0 dup keys" therefore proved only that *some* writer exercises the `on_conflict` path — overwhelmingly `verified_single_player` / Daily Fritz, which had steady volume the whole window. It did **not** show that a **live multiplayer** completion (`source_type = 'live_room'`) reaches `insertRankedGameIdempotent()`'s `on_conflict` branch at all — which is the exact claim RK-8 turns on, and the exact thing thrown into doubt by RK-9's finding that July-2026 human-vs-human MP games reached `matches`/`activity_feed` but never `ranked_games`. "The guard is functioning today" was asserted from the wrong evidence. **The fix itself stands** (flag deleted, `on_conflict` unconditional — `9aed0453`); only the verification claim was wrong. **Actual live confirmation 2026-09-07 — a real end-to-end MP match, not a row-shape query.** New probe `server/scripts/rankedMultiplayerRowProbe.ts` (GM-1 / SA-6 pattern): two throwaway `authenticated`-role users (service-key admin API), real Socket.IO to a locally-run server against prod Supabase (`MP_PRIVATE_CERT_MODE=1`, `TOURNAMENT_SCHEDULER_ENABLED=false`), `room:create` (`winningScore: 5`) → `room:join` → `player:ready` → `game:start`, then an honest full hand driven off the server's own `state:update` `legalMoves`/`canDraw` to a genuine game-over. **Two runs, both wrote exactly 2 `ranked_games` rows via `POST /rest/v1/ranked_games?on_conflict=player_id,source_match_id`.** Raw row (run 2): `{ player_id: 29e0…, opponent_id: 46a0…, player_score: 8, opponent_score: 0, game_type: "multiplayer", rating_before: 1500, rd_before: 350, rating_after: 1662.3108939063, rd_after: 290.318963717985, delta: 162.310893906303, source_type: "live_room", source_match_id: "24825891-2f59-4dd5-a264-6abbfa9d92ab", outcome: null }`. Both rows share the `source_match_id`; `source_type` = `live_room`; `rating_after`/`rd_after`/`delta` stamped by `commit_glicko_game_update` (server log line `"Real-time update complete"`); one `matches` row (`mode: online`). Net-zero cleanup verified (`leftover ranked_games: 0`, `leftover matches: 0`) — both auth users deleted (cascades `ranked_games`/`ghost_*`/`activity_feed`/`rating_periods`), the `matches` row deleted by `room_code` (its winner/loser FK is `ON DELETE SET NULL`, so it does **not** cascade), local `server/data/matches.jsonl` probe lines reverted. **Parked / undecided (NOT resolved by this probe) — backfill:** whether to reconstruct the missing historical ranked rows — ~2 months of human-vs-human *multiplayer* ranked games (the RK-9 dormancy window) plus the few days of *all* ranked games between the `2026-06-18` conflict-target index migration and the source-columns write effectively turning on. The forward path is confirmed; the historical gap's true size, whether `matches` / `activity_feed` / `mp_authority_events` / `ghost_games` hold enough to rebuild a defensible rating history, and whether it is worth doing at all, are all open. | RK-INV-2 |
...
| **RK-9** | **A game-over that permanently fails to persist has no alertable trace.** `createGameOverPersistScheduler`'s retry loop (`gameOverPersistence.ts`, `GAME_OVER_PERSIST_MAX_ATTEMPTS = 4`) catches each attempt's throw with a bare `log.warn`, and after the ceiling calls `markGameOverPersistFailed` + `emitGameOverPersistFailed` — which also only `log.warn`s (+ a socket emit + `emitMpAuthorityFunnel('private_game_over_persist_failed')`). `server/src/logger.ts` is plain pino to stdout with **no Sentry transport**, and there was **zero `Sentry.captureException`** anywhere in `gameOverPersistence.ts` or the ranking modules. So a game-over that permanently fails — a ranked rating never recorded, or a tournament bracket never advanced — pages nobody; the only durable artifact is an `mp_authority_events` row (table created 2026-09-01) an operator has to think to query. **Surfaced while investigating ranked-MP dormancy** (the RK-8 follow-up trace, 2026-09-06): the July 2026 human-vs-human games that reached `matches`/`activity_feed` but never `ranked_games` would have failed *exactly* here with no trace, and the live two-account repro to root-cause that dormancy is blind without this. | §8.1.3 (RK-8 follow-up) | **process** (observability) — cannot itself corrupt a rating, but hides an integrity-oracle / availability failure when one occurs | **certain** the trace was missing; the underlying persist failure itself is rare | any operator diagnosing a stranded ranked game or an un-advanced bracket | **FIX NOW (pure observability) — SHIPPED 2026-09-06 (commit `9bbc9958`).** `recordOperationalFailure('game_over_persist_exhausted', lastError, { roomCode, sourceMatchId, matchId, scheduledTournamentMatchId, sequence, attempts })` — the sanctioned `operationalTelemetry.ts` helper (`log.warn` + tagged `Sentry.captureException`; a global no-op when `Sentry.init`'s `enabled` is false, so no `config.isProd` guard, matching the convention in `supabaseUtils.ts` / `dailyFritzRouteErrors.ts` / the non-boot `index.ts` sites) — added **once**, only after the retry ceiling is exhausted. Transient failures that recover on retry deliberately do **not** fire it. Zero change to retry logic, success/failure semantics, or the happy path. Tests: the existing "retries then gives up" case now asserts it fires exactly once with the error + full context; the existing "recovers mid-retry" case asserts it does **not** fire. | RK-INV-4 |
| **RK-10** | **The ranked-MP `ranked_games` write dropped silently on a poisoned ghost move log — root cause of the "ranked-MP dormancy" thread.** Found 2026-09-07 during MP-JIT-1's post-deploy verification (`docs/mp-live-move-persist-latency.md`). **Symptom:** `ranked_games` has **0 rows with `source_type = 'live_room'`, ever**; most recent `game_type = 'multiplayer'` row is `2026-07-09` (all `source_type: null`). Three deployed two-account probe matches each wrote a `matches` row, fired `private_game_over_persist_succeeded {attempt: 1}`, and wrote **no** `ranked_games` row — participants' `profiles` rows present throughout — each preceded by `private_move_log_verification_failed` (*"Move claims a draw/pass but a legal play existed for the reported hand and board"*). **Trace:** `rooms.ts`'s PASS handler appended the ghost move log entry and incremented `room.ghostTurnIndex` **before** calling `applyMove({type:'pass'})` — the call that validates a pass (game-core throws *"Cannot pass when you have a legal play available"*). A client that submitted an illegal PASS (stale legal moves after the opponent moved, a double-tap, any client/server legal-move disagreement) got the action **rejected** (`game:action → {ok:false}`) but left a poisoned `branch:'pass'` entry in `room.ghostMoveLogs`. At game-over `evaluateHumanMoveLogVerification` (`gameOverPersistence.ts`) replays each seat's log with `verifyPlayerMoveLog(..., {strictHandContinuity:true})`; the poisoned entry fails → `humanGlickoEligible = false` → the `insertRankedGameIdempotent` call is **skipped** (`if (profile && … && humanGlickoEligible)`) → `persistGameOverOnce` **still returns success**. The MOVE handler already validated first (engine → then log); only PASS had the wrong order. The probe's bot reproduced this on every deployed run because the real network hop made it act on stale `state:update` frames far more often than the fast local path — which is exactly why local runs (identical code, same Supabase) wrote the row and deployed runs never did. | §8.1.3 (RK-8/RK-9 dormancy thread) | **availability** (competitive MP rating silently not recorded) — not integrity-oracle; nothing is corrupted, the game is dropped | **certain** the mechanism drops a match on any illegal PASS; real MP volume is near-zero pre-launch so real-player hit rate over the 2026-09-06→2026-09-07 window is unknown | any ranked live-room game that contained one illegal PASS action | **FIX NOW — SHIPPED 2026-09-07.** `51cf9aca`: the PASS handler calls `applyMove` first and only appends the ghost move / advances `ghostTurnIndex` on success (matches the MOVE handler). `3d7f9c48`: `recordOperationalFailure('ranked_insert_skipped_no_profile' / 'ranked_insert_returned_no_row', …)` so a future silent skip pages instead of hiding, plus the profile-fetch now distinguishes threw-vs-empty. Tests: `ghostMoveLogCapture.test.ts` (a rejected illegal PASS leaves the log empty, `ghostTurnIndex` at 0, state unmutated); `gameOverPersistence.test.ts` (skip fires telemetry, persist still succeeds). **Live-verified 2026-09-07** against deployed release `51cf9aca`: a real full two-account MP match wrote **2 `ranked_games` rows, `source_type: "live_room"`, via the `on_conflict` path**, `rating_after`/`delta` applied (1500 → 1662.31 / 1337.69), `profiles.ranked_games_played` incremented, **no `private_move_log_verification_failed`**, net-zero cleanup — the **first `source_type='live_room'` row in production, ever**. Full server suite 1300/1300, client `check:architecture` 20/20. **Follow-up (not blocking, recorded):** for the live-room path the move log is 100 % server-authored — re-verifying it with a reconstruction-based verifier that has known drift risk (GM-2) and then *silently* dropping all rating on disagreement is fragile; a future verifier drift re-opens total dormancy (now loud via RK-10's telemetry, but still dropped). Consider: a live-room verification failure is a server-bug alert, not a cheat signal, and the game should arguably still be rated or held for review rather than dropped. | RK-INV-2, RK-INV-3 |
...
- [x] RK-5 — `ghost/service.ts` consolidated onto `supabaseUtils.ts`'s shared `supabaseFetch` (2026-09-05); functionally equivalent for ghost's usage, gains the shared timeout/pooler/Sentry behaviour
- [ ] RK-3 — REVISIT IF SCALE, untouched
- [x] **RK-6 CLOSED 2026-09-08 (D-25)** — boot sweep (20s) + periodic catch-up (15 min) added to `startRankingCron()`, mirroring `dailyFritzStrandedRecovery.ts`'s reaper shape; mid-sweep-restart atomicity confirmed safe by construction (`commit_glicko_game_update` is one transaction per game, sweeps re-query `rating_after=is.null` fresh). See §8.3's RK-6 row for the full trace.
- [x] **RK-9** — a permanently-failed game-over persist had no alertable trace (bare `log.warn`, no Sentry anywhere in `gameOverPersistence.ts`). Surfaced while investigating ranked-MP dormancy (RK-8 follow-up). **FIX NOW (pure observability), SHIPPED 2026-09-06 (commit `9bbc9958`)** — `recordOperationalFailure('game_over_persist_exhausted', …)` fired **once** after the retry ceiling in `createGameOverPersistScheduler`, carrying roomCode / sourceMatchId / matchId / scheduledTournamentMatchId / error; a transient failure that recovers on retry does not fire it. No behaviour change. Tests assert both. Verified green (server `tsc -b` + full vitest 1294/1294, client `tsc -b` / lint / `check:architecture` 20/20 / vitest 1553).
- [x] **RK-8** — `insertRankedGameIdempotent()`'s dedup was `RANKED_GAMES_SOURCE_COLUMNS_ENABLED`-gated with no floor; defeated Guardrail #3 / INV-17 if the flag was ever unset. Found during Guardrail #5 Option B scoping, graded POSTURE, **then FIX NOW on human override**. **Stopgap SHIPPED 2026-09-05** (default-flip + boot check). **CLEAN FIX SHIPPED 2026-09-06** — `RANKED_GAMES_SOURCE_COLUMNS_ENABLED` deleted entirely: source-column write + `on_conflict` path now unconditional; flag helper, `index.ts` boot check, and all six consumer references removed; `hasIdempotentSource()` = `Boolean(sourceMatchId)` (the surviving conditional is the real no-match-id path — `completeGhostGame` null `matchId` — which the full unique index tolerates by design). Live re-confirm 2026-09-06: every prod `ranked_games` row since ~2026-08-10 carries `source_match_id`, 0 dup keys. Tests repurposed as reintroduction guards (`=false` asserted inert). Verified green (server build + vitest, client tsc/lint/vitest/check:architecture). `isRankedGameOutcomeColumnEnabled()` still deferred — its prod state stays unconfirmable cleanly. **CORRECTION 2026-09-07:** the "Live re-confirm 2026-09-06" line (both here and in the §8.3 row) was wrong — it counted all `ranked_games` rows without filtering `source_type`, so it never showed a *live multiplayer* completion reaching the `on_conflict` path. Retracted and replaced with a real end-to-end two-account MP match (`server/scripts/rankedMultiplayerRowProbe.ts`, 2 runs) that wrote 2 `source_type: 'live_room'` rows via `?on_conflict=player_id,source_match_id`, rating applied, net-zero cleanup. Fix unchanged; verification claim corrected. Historical backfill (~2 months of MP ranked games + a few days of all ranked games) is **parked/undecided**, not resolved — see the §8.3 RK-8 row. **CORRECTION 2026-09-07 (RK-10):** the dormancy is no longer "parked/undecided" for the forward path — **root cause found and fixed 2026-09-07** (RK-10: `rooms.ts`'s PASS handler appended the ghost move log before validating the pass, so an illegal PASS left a poisoned entry that failed the game-over move-log verification and silently skipped the `ranked_games` insert). **Live-verified**: a deployed two-account MP match now writes `source_type='live_room'` `ranked_games` rows with rating applied — the first ever. **Every ranked MP game from the dormancy start to the 2026-09-07 fix is unrated, and that gap is now closed going forward.** Backfill of the historical gap remains a separate decision — its true size is small (near-zero real MP volume pre-launch; the pre-2026-09-06 portion is RK-8's `source_type: null`, not RK-10), and whether `matches` / `activity_feed` / `ghost_games` hold enough to reconstruct a defensible rating history is open.
- [x] **RK-10** — ranked-MP `ranked_games` write dropped silently on a poisoned ghost move log (root cause of the dormancy thread). `rooms.ts` PASS handler appended the ghost move log / advanced `ghostTurnIndex` **before** `applyMove` validated the pass → a rejected illegal PASS still left a poisoned `branch:'pass'` entry → game-over move-log verification failed → `humanGlickoEligible=false` → `insertRankedGameIdempotent` skipped → `persistGameOverOnce` still reported success. **FIX NOW, SHIPPED 2026-09-07** — `51cf9aca` reorders PASS to validate-then-log (matches MOVE); `3d7f9c48` adds `recordOperationalFailure` for the now-loud skip. **Live-verified** against deployed `51cf9aca`: real two-account MP match wrote 2 `source_type='live_room'` `ranked_games` rows via `on_conflict`, rating applied — first ever in prod. Tests: `ghostMoveLogCapture.test.ts`, `gameOverPersistence.test.ts`. Full server suite 1300/1300, client `check:architecture` 20/20.
...
Async best-of-3 between two real users, each playing their own hands "vs Fritz" against a **shared server-generated seed** (`createDeterministicDoubleSixDeal`, same primitive as Daily Fritz/`rankedDealAuthority.ts`), share-code join flow, capped at `FRITZ_CHALLENGE_MAX_PARTICIPANTS`. Read the `record-game` handler (the highest-stakes endpoint) in full: it **reuses `verifyDailyFritzHand` directly** — the same transcript verifier already hardened by System 3's Step 3 and System 7's GC-5 canonical-digest fix — rather than a parallel reimplementation, so it inherits that hardening automatically. Idempotency is handled explicitly and correctly: a resubmission of an already-recorded `game_number` is detected (`existingGames.some(...)`) and replayed (`replayed: true`) rather than reprocessed; genuine concurrent/stale submissions get a `409` with `stale_revision` + the current `authority_revision`, via the same CAS-style `commitCommand`/expected-revision pattern used elsewhere in this codebase (Daily Fritz, tournaments). RPCs for this surface were already locked down in the cross-cutting security sweep (§ before "# System 1").

**Correction (2026-09-05) — the "low live usage / low-priority" verdict was wrong, and wrong in a way worth stating openly.** The original line above read: *"Low live usage (17 challenges / 9 attempts, last activity 2026-08-04) keeps this low-priority for further depth right now."* That assumed the routes were **mounted and reachable — just quiet.** They are not. **`registerFritzChallengeRoutes` is never called anywhere on `main`** (`git log main -S "registerFritzChallengeRoutes(app" -- server/src/index.ts` returns nothing; `index.ts` mounts `registerDailyFritzRoutes(app)` and no Fritz-Challenge equivalent). This §10.1.3 pass read the handler code and the prod row counts but **never checked route registration** — the same "read the code, don't check whether the code runs" gap System 3 corrected for `daily-puzzle`. Actual status: the server plumbing (`fritzChallenges.ts`, the two stores, the migrations, the RPC lockdown) was merged to `main` piecemeal across `554de39a` / `02b5e80f` / `66953fd1` / `210cfe40` / `8f0e8da9` / `3484c93a` — **server + schema only**; the client UI (`client/src/fritzChallenge/**`), the `VITE_FRITZ_CHALLENGES_ENABLED` flag, and the two `index.ts` registration lines exist only on `feat/fritz-challenge-multi` (8 commits ahead of `main`, last touched **2026-08-02**, never merged). The 17 challenges / 9 attempts are all one developer test account (`3d70f65e…`, the same account that generated the Daily Fritz test traffic) from a branch/preview deploy, not real users. So the `record-game` handler's `verifyDailyFritzHand` reuse is still a genuine positive architecture finding — but it describes **unmounted code**, not a live low-traffic feature. Recorded as its own finding below (dead-code status) and as the sixth parked pool; the `verifyDailyFritzHand` `requireStateDigests` omission from the Guardrail #4 investigation is moot while the routes are unmounted.
...
**Live finding, fixed out of band same day, CLOSED:** §11.1.5 below documents a **confirmed, reproducible, live bug** — deleting an account failed outright (`500`, raw Postgres error) for any user with an unresolved `bot_match_pending` row. Verified end-to-end against live prod, fixed (migration written and pg16-verified, same day, same urgency class as GC-5/RK-0), applied to prod by the human, then **re-verified live afterward, not assumed from the SQL editor's success message** — the exact original failure sequence (throwaway user + unresolved pending row + the real account-delete action) now succeeds, with a follow-up row count confirming both the `profiles` and `bot_match_pending` rows are actually gone. See §11.1.5 for the full before/after evidence.
...
- **SA-INV-3 — A genuinely concurrent double-completion of the same single-player match cannot double-process it.** *HOLDS as of 2026-09-08 — `withGhostCompletionLock` (`ghostCompletionLock.ts`) now guards `/api/ghost/complete`'s entire read-check-mutate-write sequence, matching Daily Fritz's `withDailyFritzAttemptLock` / Fritz Challenge's `commitCommand`. See **SA-4**, closed D-27.*
...
- **SA-INV-9 — Deleting an account removes every row that belongs to that player, and the deletion itself is never blocked by one of those rows.** *Was **UNENFORCED** — `bot_match_pending`'s FK to `profiles` had no `ON DELETE` action, confirmed by live reproduction (§11.1.5). **Now HOLDS, confirmed live in prod** (`2026-09-05_bot_match_pending_cascade_delete.sql`, pg16-verified, applied by the human, then re-verified with a full live reproduction — see §11.1.5). See **SA-6**.*
...
|---|---|---|---|---|---|---|---|
| **SA-6** | **Account deletion 500s for any user with an unresolved `bot_match_pending` row — CLOSED.** `bot_match_pending_user_id_fkey` referenced `profiles(id)` with no `ON DELETE` action. Confirmed live, then fixed same day out of band from the normal sequence (same urgency class as GC-5/RK-0): `2026-09-05_bot_match_pending_cascade_delete.sql` written, pg16-verified against a disposable local Postgres (pre-fix failure reproduced identically, migration applied twice for idempotency, post-fix delete confirmed to actually remove both rows, not just avoid an error). | §11.1.5 | **availability** (a core account-management flow fails outright for affected users) | was **medium** — any user with a match "in flight" within the last 30 min, or one whose `cleanup-stale` sweep was ever delayed | any user attempting deletion while an unresolved pending match existed | **CLOSED — applied to prod by the human, re-verified live afterward.** No RPC/direct-SQL path exists to read `pg_constraint.confdeltype` on prod (stated plainly, not glossed over) — verified instead by full behavioral reproduction: the exact original failure sequence now succeeds (`200`, was `500`/`23503`), and a follow-up `count(*)` confirms both the `profiles` and `bot_match_pending` rows are actually gone (0 remaining each), not just "no error." | SA-INV-9 |
...
| **SA-4** | **No lock or CAS guards `/api/ghost/complete` against a genuinely concurrent double-completion**, unlike Daily Fritz (`withDailyFritzAttemptLock`) or Fritz Challenge (`commitCommand`). Would double-process `completeGhostGame` (double Glicko application) and double-write activity/forfeit side effects, not just SA-3's symptom. | §11.1.6 | was **integrity-oracle** if hit (a double-applied rating change), but **availability**-adjacent in practice given how narrow the window is | was **low** — needs two truly concurrent requests for the same `matchId`, not just a sequential retry (which SA-INV-2 already covers) | the affected match's own rating change, doubled | **CLOSED 2026-09-08 (D-27).** Investigated the concurrency shape before writing the fix, per instruction: the race is a genuine in-process TOCTOU on `verifiedMatch.status`, read from `queryVerifiedSinglePlayerMatchByMatchId`'s in-memory `Map` cache before either concurrent request writes it back to `'completed'` — reachable within a single Node process (double-submit, flaky client retry, two tabs), no multi-instance needed, matching Daily Fritz's exact shape. Confirmed the same repro against the current code: two genuinely concurrent (unawaited-between) `/api/ghost/complete` calls for the same `matchId` both pass the `status !== 'completed'` check and both call `completeGhostGame` — 2 calls, 2 Glicko applications. `withDailyFritzAttemptLock`'s pattern (a generic per-key promise-chain mutex) ports directly with no wrinkle — the one design choice was giving ghost its own lock file/Map (`ghostCompletionLock.ts`) rather than sharing Daily Fritz's, for key-space isolation, not because the mechanism differs. **One nuance found, not a blocker:** the Fritz branch of `completeGhostGame` already routes through `insertRankedGameIdempotent` keyed on `sourceMatchId` (RK-2, shipped earlier) — a second *reaches-the-DB* Fritz insert for the same `matchId` was already a no-op at the DB layer before this fix. The route-level TOCTOU (both requests entering `completeGhostGame` at all, doubling its cost and the non-idempotent non-Fritz Ghost-branch `ghost_rating` read-modify-write, which is *not* protected by an `insertRankedGameIdempotent`-style guard) was the part still open — this fix closes that. `withGhostCompletionLock(matchId, …)` now wraps the route's entire auth-through-response body, mirroring `dailyFritzCompletionRoutes.ts`'s `try { await withDailyFritzAttemptLock(id, async () => {…}) } catch …` shape exactly. New `ghostCompletionLock.test.ts` (2 cases, mirroring `dailyFritzAttemptLock.test.ts`) plus 2 route-level cases in `ghostRankedDeal.test.ts`: two concurrent completions for the *same* `matchId` apply the Glicko change exactly once (verified failing pre-fix — 2 calls, 2 Glicko applications — then passing post-fix, not just asserted), and two concurrent completions for *different* `matchId`s are not serialized against each other (the lock is per-key, not global). Full server vitest (220 files / 1310 tests) + server build green. | SA-INV-3 |
...
- [x] **Guardrail #6 BUILT 2026-09-06 (D-24)** — `list_cascade_delete_manifest()` RPC + `checkCascadeDeleteCompleteness.ts` + `cascade-delete-allowlist.json` (5 entries) + 7-case negative test + `security-posture.yml` job, all mirroring Guardrail #1. Its first prod run found **AD-1** (own section past System 13): `ghost_games`/`ghost_profiles` FKs were NO ACTION not CASCADE, and `matches` FKs were NO ACTION not the SET NULL that SA-INV/D-20 ratified — a live account-deletion 500 for any established player, fixed by `2026-09-06_account_deletion_cascade_ghost_matches.sql` (human-applied, check now green, both repros verified). SA-6's `bot_match_pending` came back clean, confirming that fix landed.
- [x] **SA-6 CLOSED 2026-09-05** — migration applied to prod by the human; re-verified live rather than assumed (direct `pg_constraint` read unavailable, no RPC exposes it and this project has no direct Postgres connection — stated plainly; full behavioral reproduction used instead: the original failure sequence now succeeds, `profiles`/`bot_match_pending` row counts confirmed at 0 afterward).
- [x] **System 11 CLOSED for its full scope — same standing as Systems 1–10.** All 7 gaps resolved: SA-1, SA-2, SA-3, SA-5, SA-6 fixed and verified; **SA-7 CLOSED 2026-09-05** — the `/friends/request*` routes now translate the `friends_pair_unique_idx` race conflict to `409` instead of `500` (a follow-up cleanup pass, not part of D-20's Step-3 scope which deliberately left it as ACCEPT).
- [x] **SA-4 CLOSED 2026-09-08 (D-27)** — `withGhostCompletionLock` (mirroring `withDailyFritzAttemptLock`) now guards `/api/ghost/complete` end-to-end against a genuinely concurrent double-completion for the same `matchId`; confirmed the pre-fix race with a failing repro before fixing it, not just asserted. See §11.3's SA-4 row for the full trace.
...
- [x] Step 2 — invariants (§12.2, PL-INV-1..3) + gap list (§12.3, PL-1..PL-2) written 2026-09-05. Both gaps ACCEPT — PL-1 (journey account-detachment) recorded as a neutral fact, no forward-looking note, since journey mode is currently on hold product-wise; PL-2 (dead `LearningProfileStore` scaffolding) confirmed inert.
- [x] Step 2 → ratified **D-21** (2026-09-05).
- [x] Step 3 — **not needed.** Both gaps are ACCEPT; nothing to build.
- [x] **System 12 CLOSED — same standing as Systems 1–11.**
...
**Live finding, RK-0-class, fixed out of band same day:** §13.1.2 below documents a **confirmed, reproduced, live gap** — `matches`' own RLS `INSERT` policy permitted exactly the forgery System 11's SA-2 fixed at the application layer, but SA-2's fix lived entirely in the Express route (`/api/stats/record-match`) and did nothing to the database itself. A direct authenticated PostgREST call — bypassing the Express server completely — still inserted an arbitrary fabricated win/loss row. Reproduced live three times (to confirm, immediately before fixing, and again after prod application to confirm closure), fixed same day (same urgency class as GC-5/RK-0/SA-6), pg16-verified. **CLOSED — applied to prod and re-verified live**, including a clean `check:policy-manifest` re-run confirming Guardrail #1's newly-pinned `matches` entry now matches live reality exactly. §13.1.6's admin-route body/header inconsistency was fixed in the same pass (pure code, no migration, already shipped). See §13.1.2/§13.1.6 for the full before/after evidence.
...
**FIXED same day, out of band from the normal Step 2 → ratify → Step 3 sequence, given the live/trivially-reachable severity (same urgency class as GC-5/RK-0/SA-6).** Confirmed safe first: grepped every `INSERT` call site into `matches` in both `client/src` and `server/src`. Exactly two writers exist — `recordUserMatch.ts` and `recordPublicMatch.ts` — and both authenticate via `supabaseFetch`, which always uses the **service_role** key and therefore bypasses RLS unconditionally; neither is affected by this policy at all. The one client-side `.from('matches')` call (`stats/statsApi.ts`) is a `.select()`, not a write — **no direct client-side INSERT into `matches` exists anywhere in the real app**, confirming this policy is only ever consulted for a non-Express PostgREST call, exactly the reproduction above, never a legitimate code path.

Wrote `supabase/migrations/2026-09-05_matches_insert_guest_only.sql`, mirroring SA-2's own logic exactly at the layer that actually enforces it: `with_check` now additionally requires `winner_user_id IS NULL OR loser_user_id IS NULL` — a client can only ever insert the legitimate guest-opponent self-report shape (`MultiplayerGameShell.tsx`'s own guard means a real registered-vs-registered match never submits both ids from the client to begin with); a registered-vs-registered claim must come from `recordPublicOnlineMatch` under service role, which was never gated by this policy anyway. Also synced `supabase/schema.sql`'s canonical DDL to match.

**Reproduced the gap a second time immediately before fixing it** (fresh throwaway users, identical `201` forgery) to confirm it was still pinned exactly as described. Then pg16-verified the fix against a disposable local Postgres 16 instance, the same method as SA-6: built a minimal stand-in for Supabase's `auth.uid()` (a SQL function reading a session GUC) plus a non-superuser `authenticated` role to actually exercise RLS as (superusers bypass it) — confirmed the pre-fix policy reproduces the identical forgery locally, applied the real migration file twice (idempotent), then confirmed all four cases: the same forgery attempt now fails (`ERROR: new row violates row-level security policy` — PostgREST maps this to `403`/`42501`); a legitimate guest-opponent self-report (winner = caller, loser = null) still succeeds; the symmetric case (loser = caller, winner = null) still succeeds; and a non-participant attempting the same one-side-null shape is still rejected (the original participant check still holds, untouched).

**CLOSED — applied to prod by the human, then verified live, not assumed from the SQL editor's success message.** Reproduced the exact original forgery from this section one more time against live prod: minted two fresh throwaway users, POSTed the identical direct-REST body (both `winner_user_id`/`loser_user_id` populated, no `room_code`) using only the attacker's own JWT. Result: **`403`, `{"code":"42501", "message":"new row violates row-level security policy for table \"matches\""}`** — where this exact request previously returned `201`. Confirmed both legitimate shapes still work: a guest-opponent insert with only `winner_user_id` populated succeeded (`201`), and the symmetric case with only `loser_user_id` populated also succeeded (`201`) — both rows deleted afterward, both throwaway users deleted, net-zero. Then re-ran `npm run check:policy-manifest` against live prod: **clean — no drift on any pinned table**, `matches` now included in the 5 checked (was reporting a drift before this application). Guardrail #1's pin now matches live reality exactly, confirmed by the tool itself, not inferred.
...
- **Was still body-param (`req.body?.adminKey`), never migrated to AU-6's pattern — FIXED same day:** `POST /api/daily-fritz/generate`, `POST /api/daily-fritz/invalidate`, `POST /api/daily-fritz/reset-attempt`, `POST /api/ranking/process/:userId`, `POST /bot-matches/cleanup-stale`.
...
**Fixed same day, out of band, alongside the `matches` fix above.** All 5 routes now check `req.get('x-admin-secret')` instead of `req.body?.adminKey`, matching the already-hardened 3 exactly. Confirmed safe before changing anything: grepped `client/src` for any call site hitting these 5 routes — **none exists**; no client UI currently calls `generate`/`invalidate`/`reset-attempt`/`ranking-process`/`cleanup-stale` at all, so there was no paired client-side call to update in tandem, and nothing depended on the old body-param shape. Pure code change, no migration. Extended the existing AU-6 test harness (`dailyFritzAdminHeaderOnly.test.ts`) to cover the 3 daily-fritz POST routes: a correct secret in the body (the old accepted shape) is now `403`, and the correct secret via the header passes the admin gate. `ranking.ts`'s and `botMatches.ts`'s routes got the identical fix but have no existing test file to extend — not forcing one into existence for this, consistent with how migration-only fixes elsewhere in this plan are handled. Server suite green (213 files/1262 tests, was 213/1256), `tsc -b` clean, lint unchanged (217/68 baseline).
...
### 13.1.7 Telemetry/observability and deploy/infra

**Sentry error scrubbing (`sentryScrubbers.ts`) — confirmed actually wired, not just defined.** Traced `sentryBeforeSend` from its definition to `Sentry.init({ beforeSend: sentryBeforeSend, ... })` in `index.ts` — it genuinely runs on every outgoing event, not dead code sitting unused. `Sentry.init` itself is clean: `sendDefaultPii` is not set (defaults off, so cookies/IPs aren't attached beyond what Sentry does by default), `enabled` is gated on `isProd && a real DSN`, and `beforeSend` redacts `Authorization`/`x-admin-secret` headers, `admin_key` query-string fragments, and `access_token`/`admin_key`-named fields anywhere in a request payload (recursing into JSON-stringified nested strings too).

**One real, narrow gap, not urgent:** `isSensitiveKey`'s redaction list is exactly four field names (`access_token`, `accessToken`, `admin_key`, `adminKey`) — it does not scrub `password`, `refresh_token`/`refreshToken`, a generic `token`, or `email`. None of these currently flow through a code path that would attach them to an error's `request.data` (this app doesn't handle passwords server-side at all — auth is Supabase-hosted), so this is a defense-in-depth gap rather than a confirmed live leak: if a future error handler ever logged a request body containing one of these fields, it would reach Sentry unredacted. Worth widening the list opportunistically, not urgent.

**Graceful shutdown (`platform/gracefulShutdown.ts`) — confirmed real, bounded, and connected to what Render actually sends.** `SIGTERM`/`SIGINT` → notify connected sockets (`server:shutdown`) → stop accepting new HTTP connections → flush pending live multiplayer room sessions (`flushAllPendingLiveSessions`, a bounded 10 s timeout, not unbounded) → close the socket server → exit. This is a real, well-built mechanism, not aspirational — confirmed it's registered (`registerGracefulShutdownHandlers`) and exercises the same `roomLivePersistence.ts` flush path System 2's work already established. **Does not overlap with GM-1's finding**: that gap was about `verified_single_player_matches`' `deal_snapshot` column being missing entirely from prod, not about a buffered write needing a flush-on-shutdown step — `persistVerifiedSinglePlayerMatch` already writes synchronously at `/start` time, so this shutdown path has nothing to do with single-player match durability. Confirmed rather than assumed, since the two could easily have been conflated.

**Deploy/infra posture — consolidated here, not re-investigated.** This system's scope block asks for the Render/Vercel posture to be made "explicit and system-wide," since today it's scattered: System 1's §1.7 (T-17 closed — UptimeRobot HTTP monitor on `/ping` @ 5 min, confirmed 100% uptime over its observation window; T-18/T-19 accepted-risk), this session's own GM-1 finding (a Render redeploy is required, not just a migration apply, to reset the sticky `persistentDealSnapshotColumnAvailable` flag — confirmed by directly triggering one), and the structural fact (recorded since D-2/D-2-addendum) that the deployment is a **free-tier, single Render web dyno** — no horizontal scaling, no staging slice, matching Guardrail #5's "no canary, straight to 100% prod" finding exactly. Nothing new found by re-reading these — they already agree with each other — but this is the first time they're cited together as one system-wide statement rather than three separate systems' incidental findings.

## 13.2 Invariants

- **CC-INV-1 — A client can never insert a `matches` row claiming a registered-vs-registered outcome; only the guest-opponent self-report shape is client-insertable, and a registered-vs-registered row can only ever come from the server-authoritative service-role path.** *Was **UNENFORCED** — confirmed by live reproduction (§13.1.2). **Now HOLDS in a pg16-verified migration**, fixed same-day out of band from the normal sequence; **not yet live in prod** pending the human's SQL-editor application. See **CC-1**.*
- **CC-INV-2 — Every admin/ops endpoint accepts its secret via one consistent transport (the `x-admin-secret` header), never the request body or a query string.** *Was **UNENFORCED** for 5 of 8 routes (AU-6's original fix reached only 3). **Now HOLDS** — fixed same day, shipped (§13.1.6). See **CC-2**.*
- **CC-INV-3 — Every table with rating/leaderboard/competitive-integrity stakes that this plan has pinned to the policy manifest is checked predicate-by-predicate in CI, not just "RLS enabled."** *HOLDS for the 5 pinned tables (`matches` — newly pinned as a direct result of CC-1 — plus `ranked_games`, `rating_periods`, `room_live_sessions`, `room_match_logs`). **Deliberately partial by design** — 43 more tables carry live policies but are unpinned, visible in every CI run's own output, not silently uncovered (Guardrail #1's stated model, not a gap this system introduces).*
- **CC-INV-4 — `league_*`'s schema surface is fully decommissioned; nothing remains to audit.** *HOLDS — confirmed by full two-way table enumeration (§13.1.1), not assumed from System 5's "likely dead" language.*
- **CC-INV-5 — A public-readable RLS policy never exposes a sensitive or PII column.** *HOLDS — confirmed against live column schemas for every broadly-readable table (`profiles`, `activity_feed`, the tournament tables), not the stale checked-in `schema.sql` baseline (§13.1.4).*
- **CC-INV-6 — Every table meant to be service-role-only-write genuinely denies all client CRUD, not just the operations someone thought to write a policy for.** *HOLDS — confirmed at the raw predicate level for `ghost_games`/`ghost_profiles`/`verified_single_player_matches` (newly verified this system, not just traced via application logic under System 10) and the daily-fritz/fritz-challenge/room/`bot_match_pending`/gauntlet families (§13.1.3).*
- **CC-INV-7 — Outgoing error telemetry never carries an unredacted secret.** *HOLDS — the scrubber is genuinely wired into `Sentry.init`, not dead code (§13.1.7), and covers `Authorization`/`x-admin-secret` headers, the `admin_key` query param, and the `access_token`/`accessToken`/`admin_key`/`adminKey`/`password`/`refresh_token`/`refreshToken`/`email` field names (**CC-5 closed 2026-09-05** — the last three added as defense-in-depth; no server code path attaches any of them to an error payload today).*
- **CC-INV-8 — A server process shutdown never silently drops in-flight live multiplayer room state.** *HOLDS — confirmed real and bounded (10 s timeout, not unbounded), genuinely connected to `roomLivePersistence.ts`'s flush path, and confirmed distinct from GM-1's unrelated finding (§13.1.7).*

## 13.3 Gap list (risk-ranked)

**Scoring** (same axes as prior systems). *Severity* ∈ {**integrity-oracle**, **availability**, **latent-drift**, **process**, **cosmetic**}. *Verdict* ∈ {**FIX NOW**, **POSTURE**, **REVISIT IF SCALE**, **ACCEPT**}.

**Kept honest rather than padded**: after the two live findings (both already fixed below), what's left of this system's scope is overwhelmingly "read the actual predicate, confirmed correctly locked" — §13.1.3/§13.1.4 alone account for roughly 40 of the 48 audited tables with no finding attached to them at all. That is the intended outcome of a table-by-table audit, not a sign of insufficient effort; it's recorded as one line (**CC-6**) rather than manufactured into several to look more thorough than the evidence supports.

| ID | Gap | §13.1 ref | Severity | Likelihood | Blast radius | Verdict | Protects |
|---|---|---|---|---|---|---|---|
| **CC-1** | **`matches_insert_participant`'s RLS policy permitted a registered-vs-registered win/loss forgery, bypassing System 11's SA-2 application-layer fix entirely.** Reproduced live three times total (to confirm, immediately before fixing, and again post-fix to confirm closure). | §13.1.2 | **integrity-oracle**, narrowly scoped — same bounded blast radius SA-2 already established (cosmetic leaderboard/streak stats, not the real Glicko rating) | was **high** — trivially reachable via devtools by any authenticated user, no special access | a player's own or a victim's displayed win-streak/leaderboard standing | **CLOSED — applied to prod, verified live.** `2026-09-05_matches_insert_guest_only.sql` pg16-verified pre-apply, then applied by the human. Re-reproduced the exact original forgery live afterward: `403`/`42501`, was `201`. Both legitimate guest-opponent shapes reconfirmed working live (`201` each, cleaned up). `npm run check:policy-manifest` re-run against live prod: clean, `matches` now among the 5 checked with zero drift. | CC-INV-1 |
| **CC-2** | **5 of 8 admin-secret-gated routes still accepted the secret via the request body, never migrated to AU-6's header-only pattern.** | §13.1.6 | **process** (not the exact leak vector AU-6 fixed; moot today since `ADMIN_SECRET` remains unset in prod, re-confirmed live) | **low** today, would become real the moment `ADMIN_SECRET` is ever set | admin-secret exposure surface, if the env var is ever configured | **FIXED same day.** All 5 routes migrated to `x-admin-secret` header, matching the other 3 exactly. No client call site existed for any of the 5, so nothing else needed updating. Test coverage extended where a harness existed (3 daily-fritz routes); not forced where none did (`ranking.ts`/`botMatches.ts`). | CC-INV-2 |
| **CC-3** | **`daily_puzzles`'s admin RLS policies gate on a hardcoded email (`auth.jwt()->>'email' = 'olivermorid@gmail.com'`) instead of this app's usual `ADMIN_SECRET` pattern.** | §13.1.5 | **process** (a different, undocumented admin mechanism — not itself broken) | n/a — functionally fine today, single admin | an email change would silently break puzzle-admin access with no error surfaced anywhere | **POSTURE** — worth aligning to the `ADMIN_SECRET` pattern eventually for consistency and to make `assert_security_posture()`'s advisory checks meaningful here too (they inspect grants/RLS-enabled, not policy predicate content, so this is invisible to them today); not urgent. | — |
| **CC-4** | **Redundant/dead RLS policies**: `profiles_select_own` and `scheduled_tournament_registrations`'s "Users can read own registrations" are both fully subsumed by a broader public-read policy sitting next to them, so the narrower one enforces nothing. | §13.1.5 | **cosmetic** (the broader policy is confirmed the intentional one in both cases — §13.1.4) | n/a | none — a future reader could mistake the narrow policy for the real access boundary | **ACCEPT** — harmless; a cleanup pass would improve clarity but nothing is at risk. | — |
| **CC-5** | **Sentry's redaction key list (`isSensitiveKey`) covers 4 field names** (`access_token`/`accessToken`/`admin_key`/`adminKey`) **but not `password`/`refresh_token`/`email`.** | §13.1.7 | **process** (defense-in-depth gap; no current code path attaches any of these to an error payload) | **low** — would need a future error handler to newly log one of these fields | any such future payload would reach Sentry unredacted | **CLOSED 2026-09-05** — `isSensitiveKey` extended to `password` / `refresh_token` / `refreshToken` / `email`, kept exact-match (consistent with the existing mechanism; adjacent fields like `email_confirmed` / `password_set_at` deliberately untouched). Re-confirmed genuinely risk-free before the change: `grep -rniE 'password\|refresh_token\|refreshToken\|\bemail\b'` over non-test `server/src` returns **zero** hits — auth is entirely client-side against Supabase Auth, the server never handles credentials. Test added to the existing `sentryScrubbers.test.ts` (6 tests, was 5). | CC-INV-7 |
| **CC-6** | **The remaining ~40 audited tables, the fail-closed admin-secret posture, graceful shutdown, and the Render/Vercel deploy posture all surfaced no gap.** `ghost_games`/`ghost_profiles`/`verified_single_player_matches` confirmed correctly locked at the predicate level for the first time; the daily-fritz/fritz-challenge/room/gauntlet/`bot_match_pending`/`ranked_games` families all consistent with their owning systems' prior work; public-read policies confirmed non-sensitive; `league_*` confirmed fully absent; all 8 admin routes confirmed fail-closed today; graceful shutdown confirmed real and bounded. | §13.1.3, §13.1.4, §13.1.6, §13.1.7 | **cosmetic** (recorded as a clean-bill-of-health line, not a real gap) | n/a | n/a | **ACCEPT** | CC-INV-3, CC-INV-4, CC-INV-5, CC-INV-6, CC-INV-8 |
**Not started.** Step 2.

## 13.4 Checklist
- [x] Step 1 (RLS/grants + admin endpoints) — written 2026-09-05 (§13.1.1–§13.1.6). Full live table enumeration (48 policy-bearing tables + 7 already-closed metrics views = 55, cross-checked two independent ways) confirmed `league_*` fully absent and no table exists outside this union. `ghost_games`/`ghost_profiles`/`verified_single_player_matches` and the rest of the daily-fritz/fritz-challenge/room/gauntlet/ranked_games families all confirmed correctly locked at the predicate level. Public-read policies (`profiles`, `activity_feed`, tournament tables) confirmed non-sensitive and, for `activity_feed`, confirmed intentional against the app's own actual behavior rather than assumed. Two process findings (redundant dead policies; `daily_puzzles`' hardcoded-email admin gate). Admin/ops: full 8-route inventory, all fail-closed today (`ADMIN_SECRET` still unset, re-confirmed live).
- [x] **`matches` RLS gap CLOSED same day, out of band from the normal sequence (§13.1.2).** `matches_insert_participant`'s `with_check` tightened to guest-opponent-only, mirroring SA-2's own logic exactly at the layer that actually enforces it. Confirmed safe first (grepped every `matches` INSERT call site — both real writers are service-role, bypass RLS entirely; no direct client-side INSERT exists anywhere). Reproduced the gap a second time immediately before fixing it, then pg16-verified the fix against a disposable local Postgres instance (forgery now rejected — `403`/`42501`; both legitimate guest-opponent shapes still succeed; non-participant still rejected). `2026-09-05_matches_insert_guest_only.sql` written; `supabase/schema.sql` synced. **Not yet applied to prod.**
- [x] **Admin-route body/header inconsistency CLOSED same day (§13.1.6).** All 5 remaining routes (`generate`/`invalidate`/`reset-attempt`/`ranking-process`/`cleanup-stale`) migrated from `req.body?.adminKey` to `req.get('x-admin-secret')`, matching AU-6's already-hardened 3 routes exactly. Confirmed no client call site exists for any of the 5 (nothing to break). Pure code change, no migration. Test coverage extended for the 3 daily-fritz routes (`dailyFritzAdminHeaderOnly.test.ts`); `ranking.ts`/`botMatches.ts` have no existing test harness to extend, not forced. Server 213/1262 green, `tsc -b` clean, lint unchanged.
- [x] Step 1 (telemetry + deploy-posture) — written 2026-09-05 (§13.1.7). Confirmed `sentryBeforeSend` is genuinely wired into `Sentry.init` (not dead code) and its config is otherwise clean (`sendDefaultPii` off, prod+DSN-gated). One narrow, non-urgent gap: the redaction key list covers admin-secret/access-token shapes but not `password`/`refresh_token`/`email` (no current code path attaches them, so not a live leak). Graceful shutdown confirmed real, bounded, connected to the live-room flush path — and confirmed distinct from GM-1's finding, not overlapping with it. Render/Vercel deploy posture consolidated from three previously-separate findings (System 1's T-17..T-19, this session's GM-1 redeploy requirement, Guardrail #5's no-canary structural gap) into one system-wide statement, nothing new found. **Step 1 fully complete for System 13.**
- [x] **`matches` pinned to the policy manifest (Guardrail #1) same day**, closing the loop on CC-1: the exact pg16-verified post-fix shape, staged ahead of prod application (the manifest correctly reports a drift right now, since prod hasn't been migrated yet — confirmed by running the real check script live). `ENGINEERING_GUARDRAILS.md` updated to record this table was added as a direct result of this system's finding.
- [x] Step 2 — invariants (§13.2, CC-INV-1..8) + risk-ranked gap list (§13.3, CC-1..CC-6) written 2026-09-05, folding in CC-1 (`matches` RLS) and CC-2 (admin-route consistency) as already-fixed rather than open FIX NOW items, same pattern as SA-6 in System 11. Kept honest rather than padded: CC-6 records that roughly 40 of the 48 audited tables, the admin fail-closed posture, graceful shutdown, and deploy posture all surfaced no gap, as one line rather than manufactured into several.
- [x] Step 2 → ratified **D-22** (2026-09-05), after confirming no dangling `Protects` reference existed in §13.3's gap table. Step 3: none needed beyond what's already shipped. CC-3/CC-5 POSTURE (revisit opportunistically, not scheduled); CC-4/CC-6 ACCEPT. **First system where a fix fed directly back into an existing guardrail's enforcement mechanism in the same pass** (Guardrail #1: 4 → 5 pinned tables same-day), not a separately-scheduled follow-up guardrail the way System 11's SA-6/Guardrail #6 pairing was.
- [x] **`matches` migration CLOSED — applied to prod by the human, then verified live (GM-1/SA-6 standard), 2026-09-05.** Reproduced the exact original forgery once more: `403`/`42501` (was `201`). Both legitimate guest-opponent shapes reconfirmed working live. `npm run check:policy-manifest` re-run against live prod: clean, `matches` included with zero drift. **System 13 fully closed — same standing as Systems 1–12.**

---

# Incident 2026-09-05: Daily Fritz `/start` 500 — day-ahead published challenge stranded by a mid-window `FRITZ_POLICY_VERSION` bump

**Not a numbered system.** Found from a live outage report (`POST /api/daily-fritz/start` returning `500` for every user, confirmed in the browser console), not from the audit sequence. Recorded here as its own dated entry because it is a **genuinely new bug class** — temporal staleness of a version-stamped published artifact — distinct from every class Systems 1–13 covered. Finding ID **DF-STALE-1**.

## What broke (exact mechanism)

Daily Fritz pre-generates each day's run and its published challenge a full day ahead — the "generate tomorrow's run" step runs at the Pacific-midnight rollover. On **2026-09-04 07:02 UTC**, that step generated the `daily_fritz_runs` row and the `daily_fritz_published_challenges` row for `run_date = 2026-09-05` (`challenge_id = daily-fritz:2026-09-05:r2:s1`), stamped with the then-current **`FRITZ_POLICY_VERSION = 2`** (`fritz_policy_contract = fritz-policy-v2-deterministic-canonical-ties`), `content_digest = 41f5b13c…`.

Later the same day, an unrelated deploy (System 7 / GC-6 work, already ratified and live well before this incident) bumped the live constant to **`FRITZ_POLICY_VERSION = 3`** (`fritz-policy-v3-code-unit-canonical-ties`).

Once the calendar rolled over and that pre-generated run became "today," every `/start` call ran the transactional-authority branch in `server/src/http/routes/dailyFritzStartRoute.ts` (gated by `DAILY_FRITZ_TRANSACTIONAL_COMMANDS_ENABLED`, which is `true` in Render prod and unset in local dev — the environment difference that initially masked the repro). That branch:

1. rebuilt the challenge package with `buildDailyFritzPublishedChallenge(...)`, which **always** stamps `fritzPolicyVersion: FRITZ_POLICY_VERSION` from the *current* live constant (never from anything stored on the run) — so the freshly-built package carried `fritzPolicyVersion: 3` and a **different `content_digest` (`26c789d4…`)**;
2. called `publishDailyFritzChallenge(pkg)` → the `publish_daily_fritz_challenge` RPC (`supabase/migrations/2026-08-01_daily_fritz_published_challenges.sql`), which does `INSERT … ON CONFLICT (challenge_id) DO NOTHING`, re-reads the persisted row `FOR SHARE`, and `raise exception 'daily_fritz_challenge_identity_conflict'` if `published.content_digest <> p_content_digest OR published.package <> p_package`.

The pre-existing v2 row's digest can never equal a freshly-computed v3 digest — **even though the actual dealt content is byte-for-byte identical** (confirmed by direct structural diff: `seed`, `drawWinner`, `drawTiles`, and all 36 hands across the 3 games match exactly; the hand-generation functions derive purely from `runDate`/`gameNumber`/`handIndex`/`dealSize` and never read `FRITZ_POLICY_VERSION`). The digest differs *solely* because the version-stamp fields inside `package` changed from 2 to 3. So the RPC raised `daily_fritz_challenge_identity_conflict` on **every** `/start` call for the day, surfaced to callers as a generic masked `500` (`prodSafeError` hides the real message in production). Zero attempts succeeded against today's challenge until the fix — confirmed by an empty `daily_fritz_attempts`/`daily_fritz_attempt_operations` for that `challenge_id`.

Tomorrow's row (`2026-09-06`, generated `2026-09-05 07:02 UTC`, *after* the bump) was already stamped `fritz_policy_version: 3` and would not have recurred at the next rollover.

## Why nothing in Systems 1–13 would have caught this

This is a **temporal-staleness class**: a version-stamped artifact is written and frozen at time T under the code live at T, then *silently invalidated* by an unrelated code change between T and the artifact's own effective date T+1, and the invalidation only surfaces as an opaque failure once the artifact goes live. None of the plan's covered classes touch this shape:

- **RLS / policy (Guardrail #1, RK-0, CC-1):** access control on a table. Nothing here is an authorization failure — the RPC's own logic rejected its own valid input.
- **Cascade completeness (Guardrail #6, SA-6):** FK `on delete` behavior. Unrelated.
- **Idempotent writes for rating/leaderboard feeds (Guardrail #3, RK-1/RK-2):** the `/start` path *is* idempotent per user; the bug is not a double-write, it's a re-derivation that no longer matches a frozen snapshot.
- **Verification-strictness parity (Guardrail #4, RT-2):** call-site drift in how strictly a transcript is checked. Different axis — this is about the *published deal artifact*, not transcript verification.
- **Shared-logic duplication (Guardrail #2), staging/canary (Guardrail #5):** not applicable.
- **Daily-modes invariants (System 3, DM-INV-1..18) and Match-runtime invariants (System 9, RT-INV-1..12):** these assert that a published challenge is *internally consistent* and that its deal is *server-authoritative* — both of which held perfectly here. Neither asserts anything about a published challenge staying *compatible with the code that will later serve it*, because that failure mode was never contemplated. The published-challenges table's own immutability trigger is the closest existing invariant, and it is orthogonal — it prevents *mutation* of a published row, which is exactly what makes this class unrecoverable without a code path that tolerates the mismatch.
- **GC-5 (the nearest prior incident)** was construction-order sensitivity in a digest *within a single verification*, fixed by canonicalizing the serialization. Superficially similar (a digest mismatch on semantically-equal content) but a different root cause: GC-5 was non-determinism *at one point in time*; DF-STALE-1 is determinism that *changed between two points in time* because a version constant moved.

## The two rejected fix paths (useful history)

**(a) Correct the stale row in place** (`UPDATE daily_fritz_published_challenges SET fritz_policy_version = 3, fritz_policy_contract = …, content_digest = 26c789d4…, package = … WHERE challenge_id = 'daily-fritz:2026-09-05:r2:s1'`). A full statement was drafted and dry-run-verified (hands byte-identical, guarded on the old digest). **Rejected — blocked by `protect_daily_fritz_published_challenge()`** (`raise exception 'daily_fritz_published_challenge_is_immutable'` on any change to identity/version/digest/`package`/`published_at`; a companion trigger forbids `DELETE` outright). This trigger is deliberate: `daily_fritz_published_challenges` is the content-addressed record backing the "Same Deal — every player gets the same hand" guarantee shown in the Daily Fritz UI, and in-place mutation of an already-published challenge is precisely what it exists to prevent — even when the intent is a version-stamp-only correction and the hands are provably unchanged, the trigger has no way to know that and shouldn't be taught to. Do not disable or work around it.

**(b) Publish a new row under a corrected identity and route `/start` to it.** **Not viable without a semantically-wrong version bump.** `challenge_id` is `` daily-fritz:{runDate}:r{DAILY_FRITZ_RULES_VERSION}:s{DAILY_FRITZ_SEED_VERSION} `` (`server/src/dailyFritzIdentity.ts`) — a pure function of `runDate`, product-rules version, and seed version. It does **not** incorporate `FRITZ_POLICY_VERSION`. So every `/start` call for a given day computes the same `challenge_id` forever; minting a second, distinct ID for the same day would require bumping `DAILY_FRITZ_RULES_VERSION` or `DAILY_FRITZ_SEED_VERSION` — which would misrepresent what changed (neither the rules nor the seed changed, only a scoring/tie-break policy stamp) and is itself a reviewed, versioned code change, not a same-day hotfix.

## The fix that shipped — `f37cf0ef`

Code-only, in `dailyFritzStartRoute.ts`'s transactional branch: **fetch the day's already-published challenge first and reuse it as-is when one exists** (via the already-present `getDailyFritzPublishedChallenge(challengeId)`), instead of re-deriving and re-verifying it against whatever constants are live at call time.

- No existing row → `publishDailyFritzChallenge(freshPkg)` as before (first caller of the day creates it).
- Existing live row → use it unchanged; **`publishDailyFritzChallenge` is never called**, so the identity-conflict guard can never fire on a version-stamp drift again. A version bump between pre-generation and first `/start` can no longer strand callers for the rest of the day, no matter how many more bumps ship before the next rollover.
- Existing row with `status = 'invalidated'` while its run is *not* invalidated (an anomaly that should never occur — `invalidate_daily_fritz_challenge` updates both atomically) → explicit `409 challenge_publication_invalidated` + a `capture500` breadcrumb, rather than an opaque conflict.

Confirmed safe by full trace: `getDailyFritzPublishedChallenge` / `publishDailyFritzChallenge` / `start_daily_fritz_attempt_command` all resolve to the same `challenge_id` regardless of branch (it doesn't depend on policy version); `start_daily_fritz_attempt_command` populates `daily_fritz_attempts`' version columns from the *challenge row it looks up*, not from client input, so the attempt's stamped `fritz_policy_version` correctly matches the reused row (2), keeping `assertDailyFritzAttemptPinsPublishedChallenge` happy; and `assertValidDailyFritzPublishedChallenge` already accepts any *supported* prior policy version whose contract string matches its own version (GC-6's explicit design — "a v3 deploy must not fail to read a v2 challenge and strand an in-flight attempt"), so the reused v2 row passes validation.

**Regression tests** (`server/src/http/routes/dailyFritzStartPublishedChallengeReuse.test.ts`, 3 cases): a real current-code client hitting a challenge published under a supported *prior* policy version → `publishDailyFritzChallenge` not called, transactional start issued against the existing challenge's identity, `200`; the fresh-publish path still works when no row exists; the invalidated-anomaly path returns a clean `409`.

**Live verification (both halves):**
- **Probe:** `POST /api/daily-fritz/start` against `racehorse.onrender.com` (release `f37cf0ef`) with a throwaway auth user → **`200`**, real attempt created (`attempt_id 550b9787…`, `verified_match_id 694e3618…`), reusing `daily-fritz:2026-09-05:r2:s1`, first hand byte-identical to what was verified pre-fix. Probe user + its attempt/events/verified-match and two earlier stray repro users deleted afterward — net-zero prod state.
- **Independent confirmation via real traffic:** immediately after deploy, `daily_fritz_attempts` for `2026-09-05` showed a real user at `status = 'completed'` (a full set finished) plus another mid-play with `hand_verified` / `game_recorded` events. Real players are starting *and* completing again.

## Explicitly: no prod data change, no migration

**DF-STALE-1 is the only fix this session that touched neither prod data nor the schema.** Every other same-day out-of-band fix this session — GC-5 (a 3-row `result` jsonb correction, human-applied), RK-0 (RLS policy re-scope, human-applied then captured in a no-op migration), SA-6 (`bot_match_pending` cascade migration, human-applied), CC-1 (`matches` RLS migration, human-applied) — required a human-applied SQL-editor change. This one is a pure code deploy: `f37cf0ef`, pushed, Render auto-deployed, verified live. The stale v2 row is still sitting in `daily_fritz_published_challenges` exactly as generated — the fix routes around it rather than correcting it, which is the correct posture given the immutability guarantee.

## Guardrail #7 — BUILT (both halves), 2026-09-05

**Rule:** a version-stamped artifact published ahead of its effective date must be **reused as-is** by the serving path — never re-derived from the version constants live at serve time — and something must proactively detect a pre-generated artifact that will fail on its effective date, on the day the invalidating change ships rather than the day the calendar reaches it. Full write-up: `ENGINEERING_GUARDRAILS.md` §7.

**Behavioural half — reuse-first serving, pinned by INV-19:**
- `f37cf0ef` fixed `/start`. The same latent shape was then closed in `scheduled/dailyWarmup.ts` (nightly pre-generation — `warmOneDailyFritzRun` is `getDailyFritzPublishedChallenge`-first; `Promise.all` → `Promise.allSettled` so one date's failure doesn't skip the other) and `dailyFritzAdminRoutes.ts` (`/invalidate` reuses an existing row; `/generate`, the deliberate non-reuse publish, 409s cleanly on a pre-existing frozen row instead of an opaque RPC 500).
- **INV-19 (`check:architecture`)** — `findUnguardedPublishChallengeCallers` fails CI if any non-test module calls `publishDailyFritzChallenge(` without referencing `getDailyFritzPublishedChallenge` in the same file. File-level, INV-11/INV-13 style. Negative test in `checkArchitectureInvariants.test.ts` (5 cases). Verified: 3 caller files, all guarded.

**Proactive half — `server/scripts/checkPublishedArtifactFreshness.ts`** (wired into `security-posture.yml`'s `published-artifact-freshness` job, same schedule as `policy-manifest` / `cascade-delete`):
- Runs the serving path's own validity oracle (`toDailyFritzPublishedChallenge` + `assertValidDailyFritzPublishedChallenge`) against every future-dated published row, plus a `live`-challenge-needs-`live`-run cross-check.
- **Fails** on `unservable` (digest ≠ package, column ≠ package, `fritz_policy_version` below the supported floor, contract no longer resolves, wrong generation/rules/seed/verifier stamp, malformed hands — every `/start` for that day would 500) and `orphaned_run` (missing/invalidated run → `/start` 409s).
- **Reports, never fails** on `stale_tolerated` (a supported prior policy version the reuse path absorbs — the DF-STALE-1 scenario itself; visible on bump day, not fatal) and `missing_published_row` (a warmup gap `/start` self-heals).
- Pure diff (`diffPublishedArtifactFreshness`) tested independently (7 cases). **Verified against prod:** runs clean (exit 0), correctly classifies the still-present DF-STALE-1 row (`daily-fritz:2026-09-05:r2:s1`, `fritz_policy_version 2` vs current 3) as `info:stale_tolerated`.

**Scope — Daily Fritz only, deliberately.** `daily_fritz_published_challenges` is the only live instance of the pre-generate → publish → serve-later pipeline. **Puzzle Rush / daily-puzzle checked (2026-09-05) — NOT APPLICABLE:** live selection from `puzzle_pool` at run start, `rush_runs.config_version` is a leaderboard tag never re-verified, `is_official` is a runtime existence-check not an `ON CONFLICT` publish, the old 5-slot `daily_puzzles` ladder is decommissioned. **Scheduled tournaments** pre-seed brackets idempotently but stamp no re-derived version constant. **Fritz Challenge** has the identical class shape and worse (`/start` hard-equals the current `FRITZ_POLICY_VERSION`, no tolerance path, every open row already unstartable) — but its routes are unmounted dead code on `main` (FC-DEAD-1), so INV-19 and the detector are scoped to Daily Fritz; the tolerance path is part of any revival, not this guardrail.

## Status of the interrupted Guardrail #6 work (checked, not assumed)

The outage interrupted an in-progress build of Guardrail #6 (account-deletion cascade completeness). Current state, verified against `git`:

- **The admin-route header-only-secret work** (`dailyFritzAdminRoutes.ts` + `ranking.ts` + `botMatches.ts`, the CC-2 fix): **committed as `0e369234` and already pushed to `origin/main`** — it is live and recorded as CC-2 CLOSED in §13.4. It is **not** uncommitted; the earlier belief that it was is stale.
- **The Guardrail #6 migration** `supabase/migrations/2026-09-05_cascade_delete_manifest_rpc.sql` (the `list_cascade_delete_manifest()` RPC): **untracked, never staged, never committed to any branch.** It is the *only* item in the working tree.
- **Nothing from the Guardrail #6 work was swept into the outage-fix commit** `f37cf0ef` — that commit's diff is exactly two files (`dailyFritzStartRoute.ts` + the new test), verified via `git show --stat`.
- **The rest of Guardrail #6 was never written:** no `checkCascadeDeleteCompleteness.ts`, no `cascade-delete-allowlist.json`, no negative test, no `security-posture.yml` job, no `ENGINEERING_GUARDRAILS.md` BUILT update. The FK re-enumeration research (matches, `fritz_challenges.opponent_user_id` confirmed live, several DDL-inferred-only candidates on `scheduled_tournaments*` / `daily_fritz_events` / `fritz_challenge_events`, and the `matchmaking_matches` "not in live schema" discrepancy) exists only as in-conversation notes, not persisted anywhere.

Guardrail #6 is exactly where it was left: one uncommitted draft migration file plus unpersisted research. Nothing to untangle from the outage fix; the only open question is whether to resume it as-is.

**Update 2026-09-06:** resumed and completed — Guardrail #6 is now BUILT (D-24), and its first real run against prod immediately found **AD-1** (below), a live account-deletion bug the migration-file research had missed.

---

# Finding 2026-09-06: AD-1 — account deletion 500s for anyone with a Ghost profile, Ghost game, or completed multiplayer match

**Not a numbered system.** Found by **Guardrail #6's very first run against live prod**, ~24 hours after System 11 closed. This is a real, previously-undiscovered, currently-live bug — the same class as SA-6 (`HARDENING_PLAN.md` §11.1.5) but with a much wider blast radius — surfaced by a mechanism that reads the live catalog instead of a reference file. Finding ID **AD-1**.

## What broke

`account/routes.ts`'s `DELETE /api/account` is a bare `DELETE /auth/v1/admin/users/{id}` with **zero application-level cleanup** — it relies entirely on database-level `ON DELETE` behavior. Four foreign keys to `auth.users` in prod are `confdeltype = 'a'` (NO ACTION), which blocks the delete of the referenced user exactly as `RESTRICT` would:

| FK | prod (before fix) | should be | live rows |
|---|---|---|---|
| `ghost_games.user_id` | NO ACTION | `CASCADE` | 407 |
| `ghost_profiles.user_id` | NO ACTION | `CASCADE` | 52 |
| `matches.winner_user_id` | NO ACTION | `SET NULL` | 87 |
| `matches.loser_user_id` | NO ACTION | `SET NULL` | 87 |

So any user with a Ghost rating profile (everyone who has played Play-vs-Fritz standalone), a recorded Ghost game, or a completed multiplayer match — i.e. essentially every established player — could not delete their account. The request 500s with a raw Postgres `23503`.

**Reproduced live before writing the fix:** throwaway user + one `ghost_games` row + the exact `DELETE /auth/v1/admin/users/{id}` the account route runs → `HTTP 500`, `code 23503`, `violates foreign key constraint "ghost_games_user_id_fkey"`. Removing the row let the same delete succeed.

## Why nothing caught it before — the reference files were wrong

Every piece of documentation said this was fine:

- **`supabase/ghost.sql`** (lines 5, 13) declares both Ghost FKs as `on delete cascade`.
- **`supabase/schema.sql`** (lines 15–16) declares both `matches` FKs as `on delete set null`.
- **`account/routes.ts`'s own docstring** names `ghost_profiles` among the tables that "cascade" and asserts `matches.winner_user_id` / `loser_user_id` are "`on delete set null` — the match happened, the player is gone."
- **System 11 / SA-INV (D-20)** ratified the `matches` SET-NULL behavior as an invariant.

**All four were aspirational.** The applied DDL never matched them — `ghost.sql` and `schema.sql` are reference/greenfield files that drifted from (or never described) what was actually run against prod, the same way `friends.sql` (SA-5) and `fritz_challenges.sql` were each separately found stale this session.

The sharpest illustration: **SA-6's own fix migration** (`2026-09-05_bot_match_pending_cascade_delete.sql`) says in its header comment that `ghost_profiles` "was spot-checked against canonical DDL and confirmed correct." That spot-check was against `ghost.sql` — the wrong source of truth. `ghost_profiles` was **not** correct; it was NO ACTION the whole time. A documentation read confirmed a table that a live-catalog read would have flagged. That gap is exactly what Guardrail #6 closes, and it closed it on the first run.

## How it was found

Guardrail #6's `list_cascade_delete_manifest()` RPC (`2026-09-05_cascade_delete_manifest_rpc.sql`) queries `pg_constraint` directly for every FK targeting `profiles(id)` / `auth.users(id)` and returns each one's real `confdeltype`. `checkCascadeDeleteCompleteness.ts` diffed the 28 live FKs against `cascade-delete-allowlist.json` and reported 4 findings: `ghost_games` / `ghost_profiles` as `missing_cascade`, and `matches.winner_user_id` / `loser_user_id` as `wrong_action` (allow-list said SET NULL, live was NO ACTION). `bot_match_pending` — SA-6's table — came back **`'c'` (clean)**, confirming that fix genuinely landed at the database level.

## The fix — `2026-09-06_account_deletion_cascade_ghost_matches.sql`

Drops and recreates the four constraints with the actions everyone already believed were in place:

- `ghost_games.user_id` / `ghost_profiles.user_id` → `ON DELETE CASCADE`. Ghost is single-player vs the AI; a Ghost game record and a Ghost rating profile are the player's own data, with no other human's history to protect. Neither table has inbound FKs, so nothing ripples.
- `matches.winner_user_id` / `loser_user_id` → `ON DELETE SET NULL`. A completed PvP match is a shared historical result — the opponent's match history must survive one player's deletion. `cascade-delete-allowlist.json` records these two as the deliberate exceptions.

**pg16-verified** before hand-off: the pre-fix stand-in schema fails the delete identically; post-ALTER, deleting a referenced user cascades the `ghost_*` rows away and nulls the `matches` columns, confirmed by follow-up query.

**Verified live after the human applied it to prod (2026-09-06):**
- `npm run check:cascade-delete` → **green**: 23 cascade + 5 allow-listed (`matches` ×2 now matching), zero findings.
- Repro 1 (Ghost cascade): throwaway user + `ghost_games` row → `DELETE /auth/v1/admin/users/{id}` now returns **HTTP 200** (was 500), and a follow-up query confirms the `ghost_games` row is **actually gone** (cascade fired), not just that the delete didn't error.
- Repro 2 (`matches` SET NULL, both sides — a fresh probe, not a real completed match; arranging a genuine one safely would mean driving two real clients through a full game against prod): a probe `matches` row with real winner/loser user FKs. Deleting the winner → `HTTP 200`, row survives with `winner_user_id` null, `loser_user_id` + scores + `mode` intact. Then deleting the loser → `HTTP 200`, `loser_user_id` also null, match data still intact. Probe row cleaned up.

## Bug class

Same as SA-6: an FK to `profiles`/`auth.users` whose `ON DELETE` action silently isn't what it should be, discovered only when someone actually checks the live database rather than the DDL. SA-6 was found by a human tracing the account-deletion flow by hand during System 11; AD-1 was found by the automated check that System 11's finding motivated — on its first run, before it had been used even once in anger. The `matches` half specifically is also a **ratified-invariant violation** (SA-INV / D-20 asserted SET NULL; prod had NO ACTION), which is why the check reports it as `wrong_action` (drift from a recorded expectation) rather than `missing_cascade`.

---

# Finding 2026-09-05: FC-DEAD-1 — the Fritz-Challenge server code path is unmounted dead code on `main`

**Not a numbered system, not a bug.** Surfaced while checking whether Finding B (the `verifyDailyFritzHand` `requireStateDigests` omission) had any live exposure. It does not — because the code it lives in has never been reachable on `main`. Recorded as its own finding so the dead-code status is tracked, not buried in a pool line. Finding ID **FC-DEAD-1**.

## What's on `main` vs. what isn't

**On `main` (merged piecemeal, server + schema only):**
- `server/src/fritzChallenge.ts` (216 lines — identity/seed/share-code helpers)
- `server/src/http/routes/fritzChallenges.ts` (~650 lines — the `/api/fritz-challenges/*` handlers: create, get, join, start, next-hand, record-game)
- `server/src/http/stores/fritzChallengeStore.ts`, `fritzChallengeCommandStore.ts` (~510 lines)
- their test files (`fritzChallenges.test.ts`, `fritzChallenges.lifecycle.test.ts`, `fritzChallengeStore.test.ts`, `fritzChallengeCommandStore.test.ts`, `server/src/fritzChallenge.test.ts`)
- migrations: `2026-08-02_fritz_challenge_authority_primitives.sql`, `2026-08-02_fritz_challenge_canonical_telemetry.sql`, and the RPC-lockdown migrations (`0907d35b` / `8f0e8da9`)
- landed across `554de39a` / `02b5e80f` / `66953fd1` / `210cfe40` / `8f0e8da9` / `3484c93a` (`3484c93a` only widened `1 | 2` → `1 | 2 | 3` call sites in the v3 policy bump)

**Never on `main`:**
- **`registerFritzChallengeRoutes(app)`** — `git log main -S "registerFritzChallengeRoutes(app" -- server/src/index.ts` returns nothing. `index.ts` mounts `registerDailyFritzRoutes(app)` and has no Fritz-Challenge equivalent. **The routes are defined and unit-tested but never bound to the Express app** — every `/api/fritz-challenges/*` request 404s.
- The client UI: `client/src/fritzChallenge/**` (`FritzChallengeDialog.tsx`, `FritzChallengeRoom.tsx`, `api.ts`, a 1487-line CSS file), `client/src/config/fritzChallengeFeature.ts`, the `VITE_FRITZ_CHALLENGES_ENABLED` flag.
- These live only on **`feat/fritz-challenge-multi`** — 8 commits ahead of `main`, last commit **2026-08-02**, never merged. `git merge-base --is-ancestor 54e05e0e main` → false. The current client has **zero** references to `fritz-challenges` / `/fritz/challenge` / `FritzChallenge*` components (`outboundChallenge` / `friendChallenge` in the client is the unrelated real-time "challenge a friend to a live match" socket flow).

## The prod rows are dev traffic

17 `fritz_challenges` (all `status: open`, `policy=2 rules=1 verifier=2 gen=1`), 9 `fritz_challenge_attempts` (all `status: started`, `result: null`), spread 2026-07-26 → 2026-08-04 — **all one user**, `3d70f65e-e9a1-4c47-abb8-8bf2fc020295`, the same developer test account behind the Daily Fritz test traffic. From a branch/preview deploy while the feature was being built, not real users. No hand was ever verified through `record-game`.

## Recommendation — flag, don't touch

A future cleanup pass, same treatment as **DF-CAND-1b** (daily-puzzle likely-dead code): remove the ~2k LOC of Fritz-Challenge server code + tests from `main`, keeping `feat/fritz-challenge-multi` as the revival point; drop the `fritz_challenge*` tables **only after** confirming zero real (non-test) rows and that no other branch anyone still cares about depends on them. Not done now — removing tables and proving no dependency is a separate, deliberate piece of work, and the code is inert where it sits (unmounted routes, `service_role`-locked RPCs). If instead the feature is revived, Finding B's fix (`requireStateDigests: true`) is part of that revival.
...
| D-25 | 2026-09-08 | **RK-6 CLOSED — the ranking cron gained a boot-time sweep + a 15-min periodic catch-up, mirroring the `dailyFritzStrandedRecovery` reaper pattern (`setTimeout` boot delay, `setInterval` cadence, idempotent `scheduled` guard).** Investigated the mid-sweep-restart question RK-6 flagged as unconfirmed before writing any fix, per instruction: read `commit_glicko_game_update` (`supabase/migrations/2026-06-30_commit_glicko_game_update_rpc.sql`) — each pending game's rating update, `ranked_games.rating_after` stamp, and `rating_periods` insert are committed by **one plpgsql function call, i.e. one Postgres transaction**, and `processRatingPeriod` commits games sequentially, re-querying `rating_after=is.null` fresh on every sweep. **Conclusion: already safe by construction, no new locking needed.** A process restart mid-sweep leaves no partial state — every game already committed is already excluded from the next sweep's query (its `rating_after` is non-null), and every game not yet reached is simply untouched and picked up next time; nothing can double-apply a rating change this way (that failure mode is M1/SA-4's, a different code path — `/api/ghost/complete` with no per-request lock — not this one). The fix itself is additive only: `startRankingCron()` still schedules the unchanged Sunday `0 0 * * 0` decay+catch-up run, and now also fires `processAllPendingRatingGames()` once 20s after boot and every 15 minutes after that, both wrapped in their own try/catch so a sweep failure never takes down the interval. New `cron.test.ts` (6 cases, fake timers, `node-cron` and `periodService` mocked) pins: boot sweep fires before the first interval tick, a mid-week pending game is processed on the next periodic tick without waiting for the weekly schedule, the interval keeps running after a sweep throws, the weekly RD decay path is untouched by the catch-up path, and `startRankingCron()` is idempotent under a double call. Full server vitest (219 files / 1306 tests) + `tsc -b`-equivalent (`npm run build --prefix server`, game-core built first) green. | RK-6 was ratified REVISIT IF SCALE at D-15 — `LAUNCH_READINESS_CHECKLIST.md`'s M2 named wider traffic as the trigger condition that ratification was waiting on. Matching an existing, already-proven reaper shape (rather than inventing a new locking mechanism) kept the fix small and consistent with `dailyFritzStrandedRecovery.ts` / `reservedRoomCleanup.ts` / the tournament scheduler's own boot-tick. The atomicity investigation is recorded in full because it was the open question standing between REVISIT IF SCALE and CLOSED — a future reader should not have to re-derive that `commit_glicko_game_update` is single-transaction-per-game from scratch. |
| D-26 | 2026-09-08 | **Guardrail #5 (staging/canary before 100% prod) — direction call made, ratifying `docs/guardrail-5-staging-canary-scoping.md`'s own recommendation as written, no changes.** **Do NOT build Option C** (a second Render service as a server-side canary) at current scale: priced at ~$7+/mo of new recurring cost plus a permanent two-environment maintenance burden (schema sync, env sync, seed data, RLS/RPC mirroring, CSP edits), and — checked against the record, not assumed — it **would not have caught a single one of AD-1, SA-6, or DF-STALE-1**, the three real incidents this plan has had; for the two schema-drift incidents (AD-1, SA-6) a shadow-DB canary would have actively *masked* the bug, since the canary's own DB would carry the same drifted constraint the live-catalog check (Guardrail #6) was built to catch. **Adopt Option A** (Vercel preview deployments as the pre-merge habit for client-affecting changes; a PR-triggered smoke workflow against the preview URL is a reasonable phase-2, not required now) and **Option B, light version** (already shipped 2026-09-05 as `docs/server-feature-flags.md` — a flag manifest + the convention that materially new logic in a high-stakes server handler ships behind a default-OFF flag, proven by `RANKED_GAMES_SOURCE_COLUMNS_ENABLED`'s own history in this same file, §8.3 RK-8). **Revisit triggers, named explicitly so this isn't re-litigated from scratch:** move off Render free tier (multi-instance becomes real), **or** sustained load such that "a bad deploy is live for ~90s before the smoke test fails it" affects a material number of sessions — **~50+ concurrent users at peak, or ~1 deploy/day with real live-multiplayer usage**. Until then, "bad deploy → smoke test red in ≤2 min → roll back" (§1.3 of the scoping doc) is a small enough blast radius that the canary's cost/complexity is not justified. `ENGINEERING_GUARDRAILS.md` §5 rewritten from "NOT YET BUILT AT ALL … awaiting a direction call" to the accepted posture, carrying the same revisit triggers. **Left open, not part of this decision:** Option A's own remaining action items are human/dashboard work, not a code or doc change this decision makes — confirming Vercel preview deployments are actually on and PRs get the preview comment, and fixing the scoping doc's noted `.vercel/project.json` mislink. | The scoping work (`docs/guardrail-5-staging-canary-scoping.md`) was already complete and reached a clear, evidence-based recommendation — what was missing was not more analysis, it was someone actually making the call. §5 read as an unbuilt blocker ("NOT YET BUILT AT ALL") when the real state was "scoped, recommended, awaiting a decision" — a materially different thing for anyone reading the guardrails file to size up launch readiness against. Ratifying the doc's own conclusion rather than re-deriving a new one keeps the reasoning traceable to the evidence that produced it (three real incidents, checked one by one, none of which the rejected option would have caught) instead of restating a preference. Doc-only: no code, no CI change, no migration. |
| D-27 | 2026-09-08 | **SA-4 CLOSED — `/api/ghost/complete` gained a matchId-scoped completion lock (`withGhostCompletionLock`), mirroring `withDailyFritzAttemptLock`.** Investigated the concurrency shape before writing the fix, per instruction. **Findings:** (1) the race is real and reachable within a single Node process — no multi-instance needed — a genuine in-process TOCTOU on `verifiedMatch.status`, read from `queryVerifiedSinglePlayerMatchByMatchId`'s in-memory `Map` cache before either of two concurrent requests writes it back to `'completed'`; a double-submit, a flaky client retry, or two tabs all reach it. Reproduced directly: two genuinely concurrent (unawaited-between) `/api/ghost/complete` calls for the same `matchId` both pass the `status !== 'completed'` check pre-fix — 2 calls to `completeGhostGame`, 2 Glicko applications. (2) **`withDailyFritzAttemptLock`'s pattern ports directly, no wrinkle** — it's a generic per-key promise-chain mutex (`Map<string, Promise<void>>`) with nothing Daily-Fritz-specific in its implementation; the only design choice was giving ghost its own lock file (`ghostCompletionLock.ts`) rather than importing Daily Fritz's, for key-space isolation between an `attemptId` and a `matchId`, not because the mechanism needed to differ. (3) **One nuance surfaced, not a blocker:** the Fritz branch of `completeGhostGame` already routes its `ranked_games` insert through `insertRankedGameIdempotent` keyed on `sourceMatchId` (RK-2, shipped earlier) — a second Fritz insert for the same `matchId` was already a DB-layer no-op before this fix. What stayed genuinely open was the route-level TOCTOU itself: both requests still entered `completeGhostGame` at all (doubling its cost), and the non-Fritz Ghost branch's `ghost_rating` read-modify-write has no DB-layer idempotency guard the way the Fritz branch's rating write does — a staggered-but-overlapping double-completion there really can double-apply. The route-level lock closes both. **Fix:** `withGhostCompletionLock(matchId, async () => {…})` now wraps `/api/ghost/complete`'s entire auth-through-response body, mirroring `dailyFritzCompletionRoutes.ts`'s `try { await withDailyFritzAttemptLock(id, async () => {…}) } catch …` shape exactly (synchronous request-shape validation stays outside the lock, same as Daily Fritz). New `ghostCompletionLock.test.ts` (2 cases, mirrors `dailyFritzAttemptLock.test.ts`'s coverage of the primitive) + 2 new route-level cases in `ghostRankedDeal.test.ts`: same-`matchId` concurrent completions apply the Glicko change exactly once (confirmed failing pre-fix with the real repro above, not just asserted, then passing post-fix), different-`matchId` concurrent completions are not serialized against each other (the lock is per-key). Full server vitest (220 files / 1310 tests) + server build green. | SA-4 was ratified REVISIT IF SCALE at D-20 — `LAUNCH_READINESS_CHECKLIST.md`'s M1 named wider traffic as the trigger condition that ratification was waiting on, same call as M2/RK-6 the same day. Matching an already-proven reaper/lock shape (rather than inventing a Postgres-level CAS) kept the fix small and consistent with Daily Fritz's own precedent, and is the right tool at this deployment's structurally-single-instance scale (Render free tier, D-2 addendum) — a DB-level lock would be over-engineering for a race this codebase already has a working in-process answer to. The investigation is recorded in full, including the RK-2 nuance, so a future reader isn't left wondering whether the Fritz branch needed a second fix. |
| D-24 | 2026-09-06 | **Guardrail #6 (account-deletion cascade completeness) resumed and BUILT to completion — and its first prod run found AD-1, a real live bug.** The RPC (`list_cascade_delete_manifest()`), check script, allow-list, negative test, and CI job were all built mirroring Guardrail #1's shape. **Decisions:** (1) the allow-list carries 5 entries, not 1 — `matches.winner_user_id`/`loser_user_id` (the documented one) plus `fritz_challenges.opponent_user_id` (participant-not-owner, behaviorally verified live) and `daily_fritz_events.user_id` / `fritz_challenge_events.user_id` (append-only telemetry logs whose `attempt_id`/`challenge_id` FKs cascade). Each investigated with SA-6-level rigor: SET NULL cannot block deletion, and each is a deliberate per-column choice in its own defining migration. The `scheduled_tournament*` player/winner columns turned out to be plain `text`, not FKs (converted by `2026-05-16_zz_tournament_bot_fill.sql`); `matchmaking_matches` is absent from the live schema — neither in scope. (2) **AD-1 recorded as its own dated finding, not folded into a System 11 gap row** — it's a previously-undiscovered live bug found by a new mechanism ~24h after the numbered sequence closed, deserving the SA-6/DF-STALE-1 "what broke / why nothing caught it / how found+fixed" treatment. The reference files (`ghost.sql`, `schema.sql`), `account/routes.ts`'s docstring, and SA-INV/D-20 all claimed behavior the applied DDL never had; SA-6's own fix migration "spot-checked `ghost_profiles` against canonical DDL" — the wrong source of truth. (3) `matches`'s finding is `wrong_action` (drift from the ratified SET NULL invariant), not `missing_cascade`. Fix migration `2026-09-06_account_deletion_cascade_ghost_matches.sql` pg16-verified, human-applied, check now green, both Ghost-cascade and matches-SET-NULL repros verified live. `bot_match_pending` (SA-6) came back `'c'` — that fix genuinely landed. | The guardrail did exactly what it exists for on its first real use — a live-catalog check caught what four documentation sources and a ratified invariant all got wrong. Recorded as its own D-row (not folded into D-20) so a cold read sees Guardrail #6's completion and AD-1's discovery as one event, distinct from System 11's original SA-6. |
| D-23 | 2026-09-05 | **DF-STALE-1 (Daily Fritz `/start` outage) — fixed in the route, not the data; recorded as a new bug class outside the numbered sequence.** A day-ahead pre-generated `daily_fritz_published_challenges` row (stamped `FRITZ_POLICY_VERSION = 2`) was stranded when an unrelated same-day deploy bumped the live constant to 3 before the run date rolled over — `publish_daily_fritz_challenge`'s identity-conflict guard then 500'd every `/start` call, because a freshly re-derived v3 digest can never match the frozen v2 row even though the dealt hands are byte-identical. **Three decisions:** (1) fix in `dailyFritzStartRoute.ts` (reuse the already-published challenge, never re-verify it against current constants) rather than correcting the row — the row is protected by `protect_daily_fritz_published_challenge()`, a deliberate immutability trigger backing the "Same Deal" fairness guarantee, and must not be worked around; a new-identity publish was also rejected (`challenge_id` derives from rules/seed version, not policy version, so a second same-day ID would need a semantically-wrong version bump). (2) This is a **genuinely new class** — temporal staleness of a version-stamped published artifact, invalidated by a later unrelated change — not a variant of any Systems 1–13 class, so it gets its own dated incident section, not a System 13 gap row. (3) **Guardrail #7 framing: PARTIALLY BUILT, honest label is "the primary instance is closed by the fix."** The behavioral half (reuse-if-published) is built + regression-tested for the one known site; the class-level check and the proactive-stale-artifact detection are not built; no `ENGINEERING_GUARDRAILS.md` section written yet, pending a decision on the outstanding halves and on the Guardrail #6 status. **No prod data change and no migration — the only same-day fix this session with neither** (GC-5/RK-0/SA-6/CC-1 all needed a human-applied SQL change). | Live outage, diagnosed by reproducing the real unmasked error (`daily_fritz_challenge_identity_conflict`) against prod Supabase, tracing it to the pre-generation/version-bump interaction, and confirming the fix live (probe `200` + real players completing sets). Fix is `f37cf0ef`, pushed and deployed same day per the "live bug affecting real players, don't hold for a review cycle" rule (same as GC-5's push). Recorded as its own D-row rather than folded into D-22 so a cold read sees this was found *after* System 13 closed, from an outage, not from the audit. |
| D-22 | 2026-09-05 | **System 13 §13.2 (CC-INV-1..8) + §13.3 (CC-1..CC-6) RATIFIED as written.** CC-1 (`matches` RLS forgery) and CC-2 (admin-route body/header inconsistency) ratified in already-fixed form — same pattern as D-19/D-20 — with CC-1's migration still pending prod application, independent of this ratification. CC-3 (`daily_puzzles`' hardcoded-email admin gate) and CC-5 (Sentry's narrow redaction list) are **POSTURE**: revisit opportunistically, not scheduled. CC-4 (redundant/dead RLS policies) and CC-6 (the remaining ~40 audited tables + admin/shutdown/deploy posture, all clean) are **ACCEPT**. **Step 3: none needed beyond what's already shipped.** **First system in this plan where a fix fed directly back into a guardrail's enforcement mechanism in the same pass, not as a separately-scheduled follow-up**: Guardrail #1 went from 4 to 5 pinned tables (`matches`) same-day as CC-1's fix, unlike System 11's SA-6, which produced Guardrail #6 as a new, separate, NOT-YET-BUILT guardrail rather than extending an existing enforced one. | Human reviewed the list, same as D-3/D-9/D-10/D-13/D-14/D-16/D-19/D-20/D-21, and confirmed no dangling `Protects` reference existed in §13.3's gap table (CC-INV-7 specifically checked against CC-5's reference to it) before ratifying — recording the verification step itself, not just its result, since the request was explicit about checking rather than assuming the cross-references were consistent. |
| D-21 | 2026-09-05 | **System 12 §12.2 (PL-INV-1..3) + §12.3 (PL-1..PL-2) RATIFIED as written. System 12 CLOSED — no Step 3 needed, both gaps ACCEPT.** The human directed one framing adjustment before ratification: PL-1 (journey progress is device-local, no account tie-in) is recorded as a neutral fact with no forward-looking "revisit later" note, since journey mode is currently on hold product-wise, not being actively developed. What is ratified: 3 invariants (no server-touching state anywhere in scope, confirmed by direct grep rather than inherited from the plan's own "client-only, low risk" scoping note; journey's account-detachment; every localStorage read path fails safe); 2 gaps, both **ACCEPT** — PL-1 as above, PL-2 (`learning/profileProgress.ts`'s `LearningProfileStore` interface, confirmed to have no implementer anywhere in the codebase — inert scaffolding, not a half-built server integration). | Human reviewed the list, same as D-3/D-9/D-10/D-13/D-14/D-16/D-19/D-20 — with one explicit framing instruction (no implied urgency on PL-1) recorded verbatim so a cold read doesn't reintroduce a "revisit when cross-device sync is prioritized" note that was deliberately removed before ratification. |
...
| 2026-09-05 | **System 11 fully closed — Step 1 (live bug found), Step 2 (ratified D-20), Step 3 (SA-1/SA-2/SA-3/SA-5 shipped), Guardrail #6 written, SA-6 confirmed live after prod application.** Step 1 (§11.1.1–§11.1.6) traced social/stats/account/the non-MP game-over pipeline, surfacing a live account-deletion bug (`bot_match_pending`'s FK to `profiles` had no cascade) fixed same day out of band from the normal sequence, plus four more findings (leaderboard double-counting, a bounded self-report forgery surface, stale `friends.sql` documentation, a missing activity-feed dedupe key). Step 2 (§11.2/§11.3, 9 invariants, 7 gaps) ratified as D-20 with SA-6 already folded in as fixed. Step 3 shipped exactly the agreed scope: **SA-1** — `dedupeMatchRows()` wired into both leaderboard aggregations, confirmed against real live data (a real user's 63 raw online wins → 47 deduped). **SA-2** — reproduced the forgery live first (a fabricated win against a real throwaway victim, `200`-accepted), fixed by requiring a matching `recordPublicOnlineMatch` row for the registered-vs-registered case specifically (traced the client and found that's the only case ever forgeable — genuine registered-vs-registered matches never reach this endpoint at all), confirmed the fix blocks the exact reproduction (`409`). **SA-3** — `sourceMatchId` added at the `ghost.ts` call site. **SA-5** — `friends.sql` resynced to the live RLS policies and the live `CHECK` constraint (confirmed `'blocked'` is genuinely live-accepted, not left as an open question). SA-4/SA-7 left untouched, as scoped. Guardrail #6 (account-deletion cascade completeness) written to `ENGINEERING_GUARDRAILS.md`, same format as #1–#5, marked NOT YET BUILT. **SA-6's migration, applied to prod by the human, was then re-verified live rather than assumed** — a direct `pg_constraint` read wasn't possible (no RPC exposes it, no direct Postgres connection exists for this project — stated plainly rather than skipped), so verification used a full behavioral reproduction instead: the exact original failure sequence now succeeds, and a follow-up row count confirmed both `profiles` and `bot_match_pending` rows are genuinely gone (0 remaining), not just "no error." **System 11 is now closed for its full scope, same standing as Systems 1–10.** |
| 2026-09-05 | **System 12 (Progression & Learning) fully closed, ratified D-21 — lightest system in the plan, confirmed rather than assumed.** The "client-only, low risk" scoping note was verified by direct grep of `journey/`/`learn/`/`learning/`/`training/` plus the full server schema, not inherited: zero server-touching state anywhere in scope. 3 invariants, 2 gaps, both **ACCEPT** — journey progress's account-detachment recorded as a neutral fact per explicit instruction (journey mode currently on hold product-wise, no forward-looking note), and `learning/profileProgress.ts`'s unused `LearningProfileStore` interface confirmed inert. No Step 3 needed. |
| 2026-09-05 | **System 13 (remaining cross-cutting/infra) fully closed, ratified D-22 — the plan's numbered sequence is now complete, Systems 1–13.** Step 1 enumerated the full live table inventory two independent ways (48 policy-bearing tables + 7 already-closed metrics views), confirmed `league_*` fully absent, and read every table's actual RLS predicates rather than trusting "RLS enabled" — surfacing a live, RK-0-class finding: `matches_insert_participant`'s `with_check` permitted exactly the forgery System 11's SA-2 fixed at the application layer, but never touched at the database layer that actually enforces it (reproduced live: a direct authenticated PostgREST call, bypassing Express entirely, still forged a row). Also found 5 of 8 admin-secret-gated routes never migrated to AU-6's header-only pattern, and confirmed telemetry/deploy posture (§13.1.7) clean. Both live findings fixed same day, out of band: **CC-1** (`matches` RLS) via `2026-09-05_matches_insert_guest_only.sql`, pg16-verified pre-apply; **CC-2** (admin routes) via a pure code migration to `x-admin-secret` headers, no client call site existed for any of the 5 so nothing else needed updating. `matches` pinned to the policy manifest same-day as the fix — **the first system in this plan where a fix fed directly back into an existing guardrail's enforcement mechanism in the same pass**, rather than producing a new, separately-scheduled guardrail the way System 11's SA-6/Guardrail #6 pairing did. Step 2 (8 invariants, 6 gaps) ratified as D-22 after confirming no dangling `Protects` reference existed in the gap table; CC-3/CC-5 POSTURE, CC-4/CC-6 ACCEPT, no Step 3 needed beyond the two already-shipped fixes. **CC-1's migration, applied to prod by the human, was then re-verified live, same GM-1/SA-6 standard**: reproduced the exact original forgery a third time (`403`/`42501`, was `201`), reconfirmed both legitimate guest-opponent shapes still work (`201` each), and re-ran `npm run check:policy-manifest` against live prod — clean, `matches` now among the 5 checked tables with zero drift. **Systems 1–13 are now all closed for their audited scope.** Explicitly parked, not forgotten: System 9's 5 items (`modules/guided/`, `modules/daily-puzzle/`, `client/src/match/board/`, `useLiveMatchSession.ts`'s composed hooks, the review hooks), System 11's SA-4 (REVISIT IF SCALE) and SA-7 (ACCEPT), System 13's CC-3 and CC-5 (both POSTURE). |
| 2026-09-05 | **DF-STALE-1 — Daily Fritz `/start` outage, fixed same day (`f37cf0ef`, pushed + deployed + verified live).** Every `POST /api/daily-fritz/start` was 500ing: a `daily_fritz_published_challenges` row pre-generated on 2026-09-04 under `FRITZ_POLICY_VERSION = 2` was stranded when an unrelated same-day deploy bumped the constant to 3 before the run date rolled over — `publish_daily_fritz_challenge`'s identity-conflict guard rejected the re-derived v3 digest against the frozen v2 row on every call, despite byte-identical dealt hands. Diagnosed by reproducing the real unmasked error against prod, ruling out this session's `matches` RLS / admin-route work (neither is in the `/start` path). Fixed in `dailyFritzStartRoute.ts` — reuse the already-published challenge, never re-verify it against current constants — **not** by touching the row (protected by the `protect_daily_fritz_published_challenge()` immutability trigger) and **not** via a new-identity publish (`challenge_id` doesn't depend on policy version). Regression test added (`dailyFritzStartPublishedChallengeReuse.test.ts`, 3 cases). Verified live: probe `200` + real players completing sets. **The only same-day fix this session needing neither a prod data change nor a migration.** New bug class — temporal staleness of a version-stamped published artifact — recorded as its own incident section (not a System 13 gap) with a **Guardrail #7 proposal (PARTIALLY BUILT — the behavioral half is the fix itself; class-level + proactive-detection halves not built)**. D-23. Interrupted Guardrail #6 work confirmed intact: its draft migration is untracked/uncommitted, nothing was swept into `f37cf0ef`; the admin-route work it was paired with is separately already committed + pushed (`0e369234`). Incident section not pushed. |
| 2026-09-06 | **Guardrail #6 BUILT to completion — and its first prod run found AD-1, a real live account-deletion bug (D-24).** Built the RPC (`list_cascade_delete_manifest()`, pg16-verified), `checkCascadeDeleteCompleteness.ts`, `cascade-delete-allowlist.json` (5 entries: `matches` ×2 + `fritz_challenges.opponent_user_id` + `daily_fritz_events`/`fritz_challenge_events` telemetry logs, each SA-6-rigor investigated), a 7-case negative test, and the `security-posture.yml` job — all mirroring Guardrail #1. Human applied the RPC to prod; the first `check:cascade-delete` run flagged 4 findings: `ghost_games.user_id` / `ghost_profiles.user_id` were **NO ACTION not CASCADE**, and `matches.winner_user_id` / `loser_user_id` were **NO ACTION not the SET NULL** that `account/routes.ts`'s docstring, `schema.sql`, and ratified SA-INV (D-20) all claim — i.e. `DELETE /api/account` 500s for any user with a Ghost profile/game or a completed multiplayer match (essentially every established player). Same class as SA-6, wider blast radius. Reproduced live (throwaway user + `ghost_games` row → `23503`). Root cause: `ghost.sql` / `schema.sql` are aspirational reference files the applied DDL never matched — and SA-6's own fix migration had "spot-checked `ghost_profiles` against canonical DDL" against that wrong source. Fixed by `2026-09-06_account_deletion_cascade_ghost_matches.sql` (pg16-verified, human-applied): `ghost_*` → CASCADE, `matches` → SET NULL. **Post-apply verified live:** `check:cascade-delete` green (23 cascade + 5 allow-listed, zero findings); Ghost-cascade repro now `HTTP 200` with the row confirmed gone by follow-up count; `matches` SET NULL repro (fresh probe, both winner and loser sides) confirms the row survives with the deleted side nulled and opponent + scores intact. `bot_match_pending` (SA-6) came back `'c'` — that fix genuinely landed. AD-1 written up as its own dated section (past System 13). `ENGINEERING_GUARDRAILS.md` §6 flipped NOT YET BUILT → BUILT. 7 commits total this batch (DF-STALE-1 doc + 5 Guardrail-#6 + this docs commit), not pushed. |
| 2026-09-07 | **RK-8's "Live re-confirm 2026-09-06" conclusion corrected — it was asserted from the wrong evidence; replaced with a real end-to-end multiplayer confirmation.** The 2026-09-06 re-confirm counted all `ranked_games` rows without filtering `source_type`, so "every recent row carries `source_match_id`, 0 dup keys" only ever showed `verified_single_player` / Daily Fritz volume exercising the `on_conflict` path — never a live-multiplayer completion (`source_type = 'live_room'`), which is what RK-8 is about and what RK-9's July-2026 dormancy finding put in doubt. **The RK-8 fix stands** (flag deleted, `on_conflict` unconditional, `9aed0453`); only the verification claim was wrong. New probe `server/scripts/rankedMultiplayerRowProbe.ts` (GM-1 / SA-6 pattern): two throwaway `authenticated`-role users, real Socket.IO to a locally-run server against prod Supabase (`MP_PRIVATE_CERT_MODE=1`), `room:create` → `join` → `ready` → `game:start`, honest hand driven off the server's own `legalMoves` to a real game-over. **Two runs, each wrote exactly 2 `ranked_games` rows via `?on_conflict=player_id,source_match_id`** — `source_type: 'live_room'`, shared `source_match_id`, `rating_after`/`delta` stamped by `commit_glicko_game_update`, one `matches` row. Net-zero cleanup verified (auth users deleted → cascades; `matches` row deleted by `room_code` since its FK is `ON DELETE SET NULL`; local `matches.jsonl` reverted). §8.3 RK-8 row + §8.4 checklist updated with the retraction + the raw row. **Backfill parked/undecided** (not resolved): ~2 months of MP ranked games (RK-9 dormancy window) + a few days of all ranked games between the `2026-06-18` index migration and the source-columns write turning on — the forward path is confirmed, the historical gap's size and whether it's reconstructable/worth doing are open. Committed, not pushed. |
