...
**Picking this up cold:** there is no "next system" to start — the numbered sequence is done. **Guardrails #2, #3, #4, #6, #7 are now BUILT** (INV-16/17/18/19 in `check:architecture`, Guardrail #6's cascade check, Guardrail #7's freshness detector); **#1 was built earlier**; **#5 (staging/canary) is the only one still NOT YET BUILT** — a real infra project, not a script. Decide among: (a) one of the six parked pools above, (b) Guardrail #5, (c) something outside this plan's original scope entirely. None carry the urgency the numbered systems did — treat this file as complete for its stated purpose (Systems 1–13 hardened) unless new live evidence says otherwise.
