...
2. **System 11's SA-4 and SA-7** (D-20): SA-4 (no completion lock against a genuinely concurrent double-request on `/api/ghost/complete` — REVISIT IF SCALE, a real fix but a bigger lift than the rest of that pass, no evidence of live occurrence) and SA-7 (a friend-request race surfaces as a generic `500` instead of a clean `409` — ACCEPT, the real DB invariant already holds).
...
- [x] **System 11 CLOSED for its full scope — same standing as Systems 1–10.** All 7 gaps resolved: SA-1, SA-2, SA-3, SA-5, SA-6 fixed and verified; SA-4 REVISIT IF SCALE and SA-7 ACCEPT, both deliberately left untouched per D-20's ratified scope, not overlooked.
