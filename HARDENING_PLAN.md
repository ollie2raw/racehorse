...
- **System 6 Step 3 — FIX-NOW + safe-standalone items SHIPPED (committed
  2026-09-03, not pushed).** **AU-3:** `app.set('trust proxy', 1)` (empirically
  confirmed live: 33 reqs with a rotating `X-Forwarded-For` prefix each got a
  fresh leaderboard bucket) + `requestIp()` now returns `req.ip`; a `log.warn` on
  the 429 path (`reqIp` / `xffRaw`) is the post-deploy verification hook. **AU-4:**
...
6. **Auth / session + rate limiting** (cross-cutting) ← **Steps 1–3 DONE (D-12, D-13; Step 3 committed 2026-09-03, not pushed).** AU-3/AU-4 rate-limit bypass closed, AU-8 auth impls consolidated onto `verifyBearerToken`, AU-1/AU-6 partials shipped. *Pending human: lower Supabase JWT expiry (AU-1); AU-6 pre-`ADMIN_SECRET` checklist.*
...
(§6.1 RATIFIED D-12; §6.2 / §6.3 RATIFIED D-13; Step 3 fixes + tests committed
2026-09-03, not pushed).** AU-3/AU-4/AU-8 fixed, AU-1/AU-6 partials shipped;
AU-1 Supabase JWT-expiry + the AU-6 pre-`ADMIN_SECRET` checklist remain
human-action.
...
Status: **RATIFIED D-13 (2026-09-03). Step 3 DONE (committed 2026-09-03, not
pushed):** AU-3 ✅ (`trust proxy: 1` + `requestIp → req.ip` + 429 `log.warn`),
AU-4 ✅ (`getUserIdFromAuthHeaderSync` deleted; 4 endpoints rekeyed on `req.ip` —
option (a) taken), AU-8 ✅ (B + C consolidated onto A via `verifyBearerToken`),
AU-1 partial ✅ (cache A TTL 60→15 s; Supabase JWT-expiry still human-action),
AU-6 partial ✅ (server `?admin_key=` query acceptance removed from the 3 GET
endpoints; rest of the checklist still human-action). AU-2 / AU-5 / AU-7
unchanged. Tests: `rateLimitBypassClosed.test.ts`,
`auth/consolidatedAuthPath.test.ts`, `http/routes/dailyFritzAdminHeaderOnly.test.ts`.
...
- [x] Step 3 — fixes + tests — **DONE (committed 2026-09-03, not pushed).** **AU-3** `app.set('trust proxy', 1)` + `requestIp → req.ip` + 429-path `log.warn` verification hook (bypass confirmed live pre-fix). **AU-4** deleted `getUserIdFromAuthHeaderSync`; the 4 endpoints rekeyed on `req.ip`. **AU-8** `socialAuth` + `tournamentAuth` consolidated onto `supabaseAuth.verifyBearerToken` (tournamentAuth keeps its uuid/payload-match wrapper). **AU-1 partial** cache A TTL 60→15 s. **AU-6 partial** server `?? req.query.admin_key` removed from the 3 GET admin endpoints. Tests: `rateLimitBypassClosed.test.ts`, `auth/consolidatedAuthPath.test.ts`, `http/routes/dailyFritzAdminHeaderOnly.test.ts`. Full suite green; `tsc -b` clean; lint unchanged.
