...
- **System 7 (`@racehorse/game-core` — shared score oracle) → Steps 1–3 done
  ...
  **`FRITZ_POLICY_VERSION` 2 → 3** (contract `fritz-policy-v3-code-unit-canonical-ties`;
  ...
  game-core's own vitest suite into CI (was not running). **POSTURE:** GC-2 —
  human-action note (rollout shape in §7.1.13). **REVISIT IF SCALE:** GC-5,
  GC-3b. **ACCEPT:** GC-7. **Next: System 8.**
...
7. **`@racehorse/game-core`** — shared score oracle ← **Steps 1–3 done (§7.1 reviewed; §7.2/§7.3 RATIFIED D-14; Step 3 FIX-NOW tier committed 2026-09-04, not pushed).** GC-1+GC-9 (buildStamp + `/ready.gameCore` + smoke), GC-6+GC-8 (`localeCompare`→code-unit; **`FRITZ_POLICY_VERSION` 3**; `sortLegalMoves` pinned), GC-3a (leaf-type `expectTypeOf` guard), GC-4 (`/bot` subpath + verifier import boundary). POSTURE: GC-2 (`GAME_RULES_VERSION` rollout, human-action). REVISIT: GC-5, GC-3b. ACCEPT: GC-7.
...
**REVISIT IF SCALE / ACCEPT:** GC-5, GC-3b, GC-7 — untouched.
...
- [ ] GC-5 (REVISIT IF SCALE — authority-digest canonicalisation), GC-3b (own effort — client type unification / retire `botEngine.ts` local geometry), GC-7 (ACCEPT — delete dead Daily Puzzle DTOs with DF-CAND-1b).
