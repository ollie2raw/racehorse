...
**Two guardrails came directly out of this session's live findings**, beyond the pools above (`ENGINEERING_GUARDRAILS.md`): Guardrail #1 (RLS/policy assertions) was extended from 4 to 5 pinned tables the same day System 13 found and fixed the `matches` forgery; Guardrail #6 (account-deletion cascade completeness) was newly written after System 11's `bot_match_pending` finding and then **built to completion 2026-09-06 (D-24)** — its first prod run immediately found **AD-1**, a live account-deletion bug (`ghost_games` / `ghost_profiles` / `matches` FKs that weren't what every reference file claimed; own dated section below). **Guardrails #1 and #6 are now built and enforcing in CI; #2 through #5 are documented but marked NOT YET BUILT** (see `ENGINEERING_GUARDRAILS.md` for each one's proposed mechanism).

**One incident after closure (2026-09-05):** a Daily Fritz `/start` outage (**DF-STALE-1**, D-23) — a day-ahead pre-generated published challenge stranded by an unrelated mid-window `FRITZ_POLICY_VERSION` bump. Fixed same day, code-only (`f37cf0ef`), verified live. It surfaced a **genuinely new bug class** none of Systems 1–13 covered — temporal staleness of a version-stamped published artifact — written up as its own dated section (`# Incident 2026-09-05: …`, just before the Appendix) with a **proposed Guardrail #7** (framed PARTIALLY BUILT: the behavioral half is the shipped fix; a class-level check and proactive stale-artifact detection are not built).

**Picking this up cold:** there is no "next system" to start — the numbered sequence is done. Decide among: (a) one of the four parked pools above, (b) building one of the four NOT-YET-BUILT guardrails (#2–#5), (c) building out Guardrail #7's two outstanding halves (see the DF-STALE-1 incident section), or (d) something outside this plan's original scope entirely. None of these carry the urgency the numbered systems did — treat this file as complete for its stated purpose (Systems 1–13 hardened) unless new live evidence says otherwise.
...
## Proposed Guardrail #7 — *not built beyond the fix; framing call below*

**Rule (proposed):** a version-stamped artifact published ahead of its effective date must be re-validated for *compatibility* before being trusted — and a compatible-but-not-identical mismatch (same semantic content, drifted version stamp) must never be treated as fatal. Concretely, for any such artifact: (1) the serving path reuses the already-published artifact rather than re-deriving it against current constants (the shape of `f37cf0ef`); (2) something proactively detects a pre-generated artifact whose version stamp has fallen behind current code *before* it goes live, so it can be re-published or flagged rather than 500ing opaquely on the effective date.

**Framing call — this is "guardrail #1's `matches` extension" shaped, not "guardrail #6" shaped: PARTIALLY BUILT, and the honest label is that the primary instance is closed by the fix.** Reasoning:

- The behavioral half — *reuse-if-published, don't re-verify against moving constants* — **is now built and regression-tested** for Daily Fritz `/start`, the only place this class is known to occur. It's a real structural mechanism with tests, not a "decided to be careful" note, so per `ENGINEERING_GUARDRAILS.md`'s own bar it clears the line for being a guardrail rather than a decisions-log entry.
- What is **not** built: (i) a check that would catch a *new* code path elsewhere re-deriving-and-hard-failing against a pre-generated version-stamped artifact (there is no lint/CI rule; the fix is one call site). (ii) the proactive-detection half — e.g. a startup or pre-publish check comparing a not-yet-elapsed future-dated challenge's stamps against current constants, so drift is visible on the day of the *bump* rather than the day of *rollover*.
- **Puzzle Rush / daily-puzzle family checked (2026-09-05) — NOT APPLICABLE, not the same class.** Puzzle Rush selects puzzles *live from `puzzle_pool` at run start* (`puzzleRushStore.ts`), with no day-ahead pre-generation and no published, content-addressed, version-stamped challenge artifact. `rush_runs.config_version` is stamped from `PUZZLE_RUSH_CONFIG.version` purely as a leaderboard-era tag ("filter to comparable runs after a re-tune"), never re-verified against the current constant or used in an identity/digest guard, so a `config_version` bump strands nothing. The `is_official` daily flag is a runtime existence-check on the user's first run of the day, explicitly *not* an `ON CONFLICT` publish. The old 5-slot `daily_puzzles` ladder did have a publish path but is decommissioned (`2026-09-02_daily_puzzle_ladder_decommission.sql`, applied to prod). Daily Fritz's `daily_fritz_published_challenges` remains the only known instance of the pre-generate → publish → later-serve-and-re-verify pipeline.
- So: **not a fresh NOT-YET-BUILT entry, and not a fully-built one either.** Closest precedent is D-22's note that CC-1's fix fed straight back into Guardrail #1's enforcement in the same pass — except here the fix *is* the mechanism for the one known instance, with the class-level and proactive-detection halves outstanding. **No `ENGINEERING_GUARDRAILS.md` section written yet** — that waits on the decision about whether to build the two outstanding halves, and on the Guardrail #6 status being sorted (below).
