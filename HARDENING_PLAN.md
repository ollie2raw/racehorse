...
**A sixth pool, added 2026-09-05 from the Guardrail #4 investigation:** **`verifyDailyFritzHand`'s `requireStateDigests` omission on the Fritz-Challenge path** — an RT-2-shape asymmetry: `fritzChallenges.ts:407` and `:515` call the verifier without `requireStateDigests`, which the Daily Fritz path pins to `true` via the authority contract it writes at `/start`. Root cause is a copy-paste (the routes reuse `verifyDailyFritzHand` but never ported the contract mechanism), not a considered decision. **Confirmed zero production exposure:** the feature has never shipped past challenge-link generation — 17 `fritz_challenges` rows all `open`, 9 `fritz_challenge_attempts` all `started`/`result: null`, one test account, last activity 2026-08-04, no hand ever verified through this path. Fix path already found: pass `requireStateDigests: true` at those two call sites, or port the authority-contract pinning into `fritzChallenges.ts` `/start` — whichever this route's owner prefers when the feature is next touched. Not independently exploitable even if activated as-is (the server's engine-legality + Fritz-policy-parity reconstruction do not trust the client's state); the digest is defence-in-depth. See `ENGINEERING_GUARDRAILS.md` §4's `verifyDailyFritzHand` scoping paragraph.
