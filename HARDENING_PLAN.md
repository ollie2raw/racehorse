...
3. **System 13's CC-3 and CC-5** (D-22): CC-3 (`daily_puzzles`' admin RLS gates on a hardcoded email instead of this app's `ADMIN_SECRET` pattern — POSTURE) and CC-5 (Sentry's redaction key list doesn't cover `password`/`refresh_token`/`email`, though nothing currently sends them — POSTURE). Both worth doing opportunistically next time those files are touched, not worth a dedicated pass on their own.
...
- **CC-INV-7 — Outgoing error telemetry never carries an unredacted secret.** *HOLDS for the secret shapes this app actually has (`Authorization`/`x-admin-secret` headers, `admin_key`/`access_token`-named fields) — confirmed the scrubber is genuinely wired into `Sentry.init`, not dead code (§13.1.7). **Narrower than it could be**: doesn't cover `password`/`refresh_token`/`email`, though nothing in this codebase currently attaches them to an error payload, so this is defense-in-depth, not a live gap. See **CC-5**.*
...
| **CC-5** | **Sentry's redaction key list (`isSensitiveKey`) covers 4 field names** (`access_token`/`accessToken`/`admin_key`/`adminKey`) **but not `password`/`refresh_token`/`email`.** | §13.1.7 | **process** (defense-in-depth gap; no current code path attaches any of these to an error payload) | **low** — would need a future error handler to newly log one of these fields | any such future payload would reach Sentry unredacted | **POSTURE** — worth widening the list opportunistically next time this file is touched; not worth a dedicated pass on its own. | CC-INV-7 |
