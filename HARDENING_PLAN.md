...
- [ ] **RK-8** — `insertRankedGameIdempotent()`'s dedup is `RANKED_GAMES_SOURCE_COLUMNS_ENABLED`-gated with no floor; defeats Guardrail #3 / INV-17 if the flag is ever unset. Traced + graded **POSTURE** 2026-09-05 (found during Guardrail #5 Option B scoping). Live state: flag ON in prod, guard working, 0 dup keys in the recent 118-row window. Not fixed — awaiting a urgency call. Stopgap = default the flag `true`; clean fix = make the write + `on_conflict` unconditional and delete the flag.
