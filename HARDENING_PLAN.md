...
- [ ] GM-1 — migration applied by the human (2026-09-05). **Redeploy/restart + live post-restart confirmation (fresh match's `deal_snapshot` round-trips through a real DB read) still outstanding** — blocked on triggering a Render redeploy, which this session has no API/CLI access to do.
