...
| 8 — Ranking / Glicko-2 | Closed **for audited scope only** — RK-3/RK-5/RK-6 ratified REVISIT IF SCALE, still untouched | D-15 |
...
**A fourth pool, found while writing this refresh, not one of the three named when this refresh was requested — surfaced rather than silently left out:** **System 8's RK-3, RK-5, RK-6** (D-15, §8.3), all ratified **REVISIT IF SCALE** and still untouched (§8.4's own checklist marks them open, unlike RK-1/RK-2/RK-4/RK-7 which shipped or were resolved). RK-3 (client-side rating-prediction math omits the server's forfeit-outcome override — cosmetic-adjacent, mispredicts a number nobody currently relies on), RK-5 (`ghost/service.ts` duplicates `supabaseUtils.ts`'s `supabaseFetch` helper — code hygiene, no live behavioral gap), RK-6 (the weekly rating-period cron has no boot-time catch-up sweep — availability, low-medium likelihood, matches a pattern this codebase uses elsewhere for other reapers). None urgent; grouped here so a cold reader sees the full picture rather than only the three most recently surfaced.
