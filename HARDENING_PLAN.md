...
- [~] **RK-8** — `insertRankedGameIdempotent()`'s dedup was `RANKED_GAMES_SOURCE_COLUMNS_ENABLED`-gated with no floor; defeated Guardrail #3 / INV-17 if the flag was ever unset. Found during Guardrail #5 Option B scoping, graded POSTURE, **then FIX NOW on human override**. **Stopgap SHIPPED 2026-09-05** — flag now `getEnvBool(..., true)` (default inverted, escape hatch intact), `index.ts` boot log + Sentry on a false resolution, tests for the new default + the escape hatch. Verified green (server build + vitest, client tsc/lint/vitest/check:architecture). `isRankedGameOutcomeColumnEnabled()` left as-is (couldn't confirm its prod state cleanly). **Still open at §8.3 priority:** the clean fix — delete the flag, make the write + `on_conflict` unconditional.
