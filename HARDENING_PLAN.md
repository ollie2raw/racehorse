...
- [x] **System 11 CLOSED for its full scope — same standing as Systems 1–10.** All 7 gaps resolved: SA-1, SA-2, SA-3, SA-5, SA-6 fixed and verified; SA-4 REVISIT IF SCALE (still parked); **SA-7 CLOSED 2026-09-05** — the `/friends/request*` routes now translate the `friends_pair_unique_idx` race conflict to `409` instead of `500` (a follow-up cleanup pass, not part of D-20's Step-3 scope which deliberately left it as ACCEPT).
