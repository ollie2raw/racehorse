...
- **System 2 MP-G6 (Tier B)** — **verified 2026-09-01: `room_command_receipts`
  and `mp_authority_events` CONFIRMED absent from prod** (service_role
  `PGRST205`, not in OpenAPI, no posture trace; both source migrations exist +
  committed but never applied — 5th/6th drift instance). Fix migration
  `2026-09-01_apply_room_command_receipts_and_mp_authority_events.sql` **written +
  pg16-verified** (creates both + the funnel view; adds the missing grants;
  drops the stale `event` CHECK that was 4 names behind the code; self-asserts).
  SQL + expected output printed to the human 2026-09-01. **Open: human applies
  it in the SQL editor.** **Next after that: System 2 Step 5 (tests prove
  closure).**
...
|---|---|---|---|---|---|
| **MP-G6** — **CONFIRMED unapplied 2026-09-01; fix migration written, awaiting prod apply** | **Both `room_command_receipts` and `mp_authority_events` DO NOT EXIST in prod.** Confirmed 2026-09-01: `service_role` GET → `PGRST205` for both (service_role has broad `public` grants — a stale schema cache is ruled out because `room_live_sessions`/`room_match_logs` resolve fine); neither table nor the `mp_authority_funnel_metrics` view is in the PostgREST OpenAPI spec; `assert_security_posture()` (server-side, reads `pg_catalog`) shows no trace. Both source files exist and are committed (`2026-08-01_room_command_receipts.sql` @ `5947dd36`, `2026-08-20_mp_authority_events.sql` @ `420be2b7`) — **never applied** (no CI migration runner; 5th/6th drift instance). Effects: `withGameActionIdempotency` runs degraded to the `room_shell.actionReceipts` embedded snapshot only (a shell-trim under a reconnect storm could drop a receipt → a replayed `game:action` double-applies — low prob, silent); the durable `mp.authority` funnel is dead, telemetry is `console.info`-to-stdout only (this is *why* MP-G5 was unmeasurable). | `roomCommandReceiptStore`, `mpAuthorityEventStore` | player-visible-bug (latent, silent) + lost observability | **`2026-09-01_apply_room_command_receipts_and_mp_authority_events.sql` written + pg16-verified** (creates both tables + the view; corrects the originals — explicit grants on `room_command_receipts`, **drops the stale `event` CHECK** on `mp_authority_events` which was 4 event names behind the code; self-asserting). **Awaiting human prod apply.** | MP-INV-15 |
...
- [~] **MP-G6 (Tier B) — verified 2026-09-01: both tables CONFIRMED absent from
  prod** (service_role `PGRST205`, not in OpenAPI, no `assert_security_posture()`
  trace). Fix migration `2026-09-01_apply_room_command_receipts_and_mp_authority_events.sql`
  written + pg16-verified. **Open: human applies it in the SQL editor** (SQL +
  expected output printed to terminal 2026-09-01).
- [ ] Tiers C–E — later
