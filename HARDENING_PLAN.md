...
- **System 7 (`@racehorse/game-core` — shared score oracle) → Steps 1–3 done
  ...
  **GC-5 re-ranked FIX NOW + fixed, same day, on a live incident.** A completed,
  ...
  live): of 12 events / 8 attempts, 3 mechanically verify clean and are queued
  (not yet applied) to restore to `verified`; 2 had already self-healed; 1 never
  completed; 1 predates transcript archival; 1 (`538bfeb1…`) has a 4-entry
  cascade and was deliberately excluded pending its own investigation. **POSTURE:**
  GC-2 — human-action note (rollout shape in §7.1.13). **REVISIT IF SCALE:**
  GC-3b. **ACCEPT:** GC-7. **Pending human: go-ahead on the 3-attempt retroactive
  fix (diff shown in-session, not applied).** **Next: System 8.**
...
7. **`@racehorse/game-core`** — shared score oracle ← **Steps 1–3 done (§7.1 reviewed; §7.2/§7.3 RATIFIED D-14; Step 3 committed 2026-09-04, not pushed).** GC-1+GC-9 (buildStamp + `/ready.gameCore` + smoke), GC-6+GC-8 (`localeCompare`→code-unit; **`FRITZ_POLICY_VERSION` 3**; `sortLegalMoves` pinned), GC-3a (leaf-type `expectTypeOf` guard), GC-4 (`/bot` subpath + verifier import boundary), **GC-5 (re-ranked FIX NOW same-day on a live incident — 12 confirmed false-positive leaderboard demotions since 2026-08-01 — and fixed: authority digest now construction-order-independent, `DAILY_FRITZ_AUTHORITY_STATE_DIGEST_VERSION` 1→2)**. POSTURE: GC-2 (`GAME_RULES_VERSION` rollout, human-action). REVISIT: GC-3b. ACCEPT: GC-7. *Pending human: go-ahead on the 3-attempt retroactive re-verification diff (scratchpad `gc5-retro-reverify.ts`, not applied).*
...
- [x] **Retroactive re-verification investigated — 3 of 12 events safe, applied a targeted fix, not a blanket sweep.** Read-only script replayed each event's *actual archived transcript* through the unmodified `verifyDailyFritzHand` with `preStateDigest` stripped (every check except the digest ran). Of 12 events / 8 distinct attempts: 1 never completed (moot), 2 self-healed already (`verified`), 1 predates transcript archival (unconfirmable), 1 has a 4-entry cascade (1 mismatch + 3 derivative `missing_hand_start_progress`) and was deliberately **excluded** pending separate investigation, and **3 mechanically verify clean on every other ground** — `3a23cb9b…` (2026-08-24), `91fadc29…` (2026-08-26), `6eba765e…` (2026-09-04, the incident that surfaced this). Dry-run diff prepared (scratchpad `gc5-retro-reverify.ts`) for exactly those 3 `daily_fritz_attempts.result.verification_status → 'verified'`, with an audit-trail field recording why. **NOT applied — awaiting explicit go-ahead** (see the diff in-session; this is a data correction, not a migration, but treated with the same "human applies" caution).
