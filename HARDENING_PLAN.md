...
**Live finding, RK-0-class, fixed out of band same day:** §13.1.2 below documents a **confirmed, reproduced, live gap** — `matches`' own RLS `INSERT` policy permitted exactly the forgery System 11's SA-2 fixed at the application layer, but SA-2's fix lived entirely in the Express route (`/api/stats/record-match`) and did nothing to the database itself. A direct authenticated PostgREST call — bypassing the Express server completely — still inserted an arbitrary fabricated win/loss row. Reproduced live (twice — once to confirm the finding, once immediately before fixing it), fixed same day (same urgency class as GC-5/RK-0/SA-6), pg16-verified. **Migration written, not yet applied to prod.** §13.1.6's admin-route body/header inconsistency was fixed in the same pass (pure code, no migration, already shipped). See §13.1.2/§13.1.6 for the full before/after evidence.
...
|---|---|---|---|---|---|---|---|
| **CC-1** | **`matches_insert_participant`'s RLS policy permitted a registered-vs-registered win/loss forgery, bypassing System 11's SA-2 application-layer fix entirely.** Reproduced live twice (once to confirm, once immediately before fixing). | §13.1.2 | **integrity-oracle**, narrowly scoped — same bounded blast radius SA-2 already established (cosmetic leaderboard/streak stats, not the real Glicko rating) | was **high** — trivially reachable via devtools by any authenticated user, no special access | a player's own or a victim's displayed win-streak/leaderboard standing | **FIXED same day, out of band.** `2026-09-05_matches_insert_guest_only.sql` written, pg16-verified (forgery rejected post-fix, both legitimate guest-opponent shapes still succeed, non-participant still rejected). Pinned to the policy manifest immediately after (Guardrail #1) so the same drift can't recur silently. **Not yet applied to prod.** | CC-INV-1 |
...
- [ ] `matches` migration application to prod — pending the human; once applied, GM-1/SA-6-style live reproduction + policy-manifest reconfirmation before push.
