...
export type PlayerReviewAction =
...
export class ReviewSnapshotRecorder {
  ...
  /**
   * Capture one player decision at the true pre-action boundary.
  ...
  recordPlayerDecision(
    ...
    action: PlayerReviewAction,
    enabled: boolean,
  ): ReviewPositionSnapshotV2 | null {
    if (!enabled) return null;
    if (preState.currentPlayer !== 'you') return null;
    if (preState.handOver || preState.gameOver) return null;

    try {
      const actionNumber = this.nextActionNumber;
      const decisionId = `${this.sessionId}:you:${actionNumber}`;
      const command = buildPlayerReviewGameCommand(preState, action, decisionId);
