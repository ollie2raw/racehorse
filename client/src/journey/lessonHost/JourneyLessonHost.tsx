  ...
  return (
    <div className="journey-lesson-host">
      ...
      <main className="journey-lesson-host__main">
        ...
        {stage.kind === 'board_demo' && activeScenario ? <section className="journey-lesson-host__lesson-stage" aria-live="polite"><p className="journey-lesson-host__instruction">{activeDemoStep?.caption ?? (activeScenario.interaction.kind === 'move' ? 'Read the two playable ends before choosing a tile.' : `Count every known ${activeScenario.interaction.targetPip}-tile on the board and in your hand. The rest are unseen, not automatically Fritz’s.`)}</p>{activeDemoStep?.boardDescription ? <p className="journey-lesson-host__demo-description">{activeDemoStep.boardDescription}</p> : null}<p className="journey-lesson-host__eyebrow journey-lesson-host__zone-label">The Board</p><div className="journey-lesson-host__board"><Board board={activeDemoStep?.board ?? activeScenario.board} legalMoves={[]} selectedTile={null} highlightedEnds={activeDemoStep?.highlightedEnds ?? [activeScenario.board.leftEnd, activeScenario.board.rightEnd]} onPositionClick={() => undefined} fitMode="guided" showZoomTray={false} staticView tileSize={78} /></div>{demoSteps.length > 0 ? <div className="journey-lesson-host__demo-controls"><span aria-label={`Demonstration step ${demoStep + 1} of ${demoSteps.length}`}>Step {demoStep + 1} / {demoSteps.length}</span><button type="button" disabled={demoStep === 0} onClick={() => setDemoStep((step) => Math.max(0, step - 1))}>Back</button>{demoStep < demoSteps.length - 1 ? <button type="button" onClick={() => setDemoStep((step) => step + 1)}>Next</button> : <button type="button" onClick={continueStage}>Continue to Decision</button>}</div> : <button type="button" onClick={continueStage}>{activeScenario.interaction.kind === 'move' ? 'I see the open ends' : 'I see the known tiles'}</button>}</section> : null}
        {stage.kind !== 'frame' && stage.kind !== 'board_demo' && activeScenario ? <section className="journey-lesson-host__lesson-stage" ref={boardFocusRef} tabIndex={-1} data-stage-id={stage.id} data-scenario-id={activeScenario.id}><p className="journey-lesson-host__instruction">{activeScenario.interaction.kind === 'move' ? (stage.kind === 'mastery' ? 'Read the whole position. Choose the move you can defend.' : 'What does your move leave open?') : activeScenario.interaction.prompt}</p><p className="journey-lesson-host__eyebrow journey-lesson-host__zone-label">The Board</p><div className="journey-lesson-host__board"><Board board={currentScenarioBoard} legalMoves={session.phase === 'awaiting_result' ? legalMoves : []} selectedTile={session.phase === 'awaiting_result' ? selectedTile : null} highlightedPosition={session.phase === 'awaiting_result' ? selectedPosition : null} highlightedEnds={currentScenarioBoard ? [currentScenarioBoard.leftEnd, currentScenarioBoard.rightEnd] : [activeScenario.board.leftEnd, activeScenario.board.rightEnd]} onPositionClick={setSelectedPosition} fitMode="guided" showZoomTray={false} tileSize={78} /></div>{session.phase === 'awaiting_result' && activeScenario.interaction.kind !== 'move' ? <div className="journey-lesson-host__count" role="radiogroup" aria-label={`Count tiles containing ${activeScenario.interaction.targetPip}`}><p>How many {activeScenario.interaction.targetPip}-tiles remain unseen?</p>{activeScenario.interaction.options.map((count) => <button key={count} type="button" role="radio" aria-checked={selectedCount === count} className={selectedCount === count ? 'is-selected' : ''} onClick={() => setSelectedCount(count)}>{count}</button>)}</div> : null}{session.phase === 'awaiting_result' && activeScenario.interaction.kind !== 'pip_count' ? <><p className="journey-lesson-host__eyebrow journey-lesson-host__zone-label">Your Hand</p><div className="journey-lesson-host__hand" aria-label="Your hand">{activeScenario.playerHand.map((tile) => { const playable = legalMoves.some((move) => move.type === 'play' && move.tile && tileEquals(move.tile, tile)); return <DominoTile key={`${tile.low}-${tile.high}`} tile={tile} size={64} selected={Boolean(selectedTile && tileEquals(selectedTile, tile))} unplayable={!playable} disabled={!playable} onClick={() => setSelectedTile(tile)} />; })}</div>{selectedTile ? <div className="journey-lesson-host__placement" aria-label="Choose an open end">{selectedMoves.map((move) => move.type === 'play' && move.position ? <button key={move.position} type="button" aria-pressed={selectedPosition === move.position} className={selectedPosition === move.position ? 'is-selected' : ''} onClick={() => setSelectedPosition(move.position!)}>Play on {move.position === 'left' ? 'left end' : 'right end'}</button> : null)}</div> : null}</> : null}{session.phase === 'awaiting_result' ? <button type="button" className="journey-lesson-host__submit" disabled={activeScenario.interaction.kind === 'pip_count' ? selectedCount === null : !selectedTile || !selectedPosition || (activeScenario.interaction.kind === 'pip_count_then_move' && selectedCount === null)} onClick={activeScenario.interaction.kind === 'pip_count' ? submitCount : submitAction}>{activeScenario.interaction.kind === 'pip_count' ? 'Submit count' : 'Submit move'}</button> : null}</section> : null}
        {session.phase === 'feedback' && feedback ? <section className="journey-lesson-host__feedback" ref={feedbackFocusRef} tabIndex={-1} aria-live="polite"><p className="journey-lesson-host__feedback-label">Board consequence</p><h2>{feedback.heading}</h2><p>{feedback.explanation}</p>{feedback.fritzRemark ? <p className="journey-lesson-host__fritz-line">{feedback.fritzRemark}</p> : null}{resultBoard ? <div className="journey-lesson-host__consequence" aria-label="Move consequence"><span>Resulting open ends (highlighted on the board above): {resultBoard.leftEnd} and {resultBoard.rightEnd}</span>{lastConsequence ? <><span>Played: {lastConsequence.action.tile.low}-{lastConsequence.action.tile.high}</span><span>Playable tiles remaining: {lastConsequence.remainingPlayableTileCount}</span><span>Immediate score: {lastConsequence.immediateScoreDelta}</span>{lastConsequence.handExited ? <span>Hand exited: Yes</span> : null}</> : null}</div> : null}<button type="button" onClick={acknowledgeFeedback}>{session.feedback?.outcome === 'failed' && stage.kind !== 'frame' && stage.kind !== 'board_demo' ? (stage.retry.kind === 'fresh_variant' ? 'New Position' : 'Try Again') : 'Continue'} <RotateCcw size={16} aria-hidden="true" /></button></section> : null}
