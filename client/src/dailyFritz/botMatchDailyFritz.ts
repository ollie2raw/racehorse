/**
 * Daily Fritz helpers used by BotMatchScreen.
 *
 * Isolated here so BotMatchScreen module-level scope stays focused
 * on match orchestration rather than Daily Fritz lifecycle details.
 *
 * These are BotMatchScreen-specific Daily Fritz concerns —
 * not general Daily Fritz API (see dailyFritz/api.ts).
 */

import type { BotMatchState } from '../bot/botEngine';
import { BOT_MATCH_DEBUG_ENV, hasDebugLocalStorageFlag } from '../bot/botMatchDebug';
import { normalizePreGameDrawTile } from '../match/preGameDraw/preGameDrawLogic';
