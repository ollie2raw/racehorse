/** Client copy of server ranking math — keep in sync with server/src/ranking/glicko2.ts */
...
export const FRITZ_ROOKIE_RATING = 800;
export const FRITZ_ROOKIE_RD = 50;
export const FRITZ_STANDARD_RATING = 1200;
...
export function getFritzConfig(fritzId: string): {
  ...
  if (fritzId === FRITZ_ROOKIE_ID) {
    return { rating: FRITZ_ROOKIE_RATING, rd: FRITZ_ROOKIE_RD };
  }
  if (fritzId === FRITZ_STANDARD_ID) {
    return { rating: FRITZ_STANDARD_RATING, rd: FRITZ_STANDARD_RD };
  }
  if (fritzId === FRITZ_GRANDMASTER_ID) {
    return { rating: FRITZ_GRANDMASTER_RATING, rd: FRITZ_GRANDMASTER_RD };
  }
  if (fritzId === FRITZ_MASTER_ID) {
    return { rating: FRITZ_MASTER_RATING, rd: FRITZ_MASTER_RD };
  }
  return { rating: FRITZ_RATING, rd: FRITZ_RD };
}
