...
function createMatchState(puzzleDateSeed: string): { puzzle: ReturnType<typeof getPuzzleForSeed>; state: BotMatchState; progress: DailyPuzzleProgress } {
...
export default function DailyPuzzleScreen({ onBack }: DailyPuzzleScreenProps) {
  ...
  const finishResult = (nextResult: PuzzleResult, finalMoves: number) => {
    ...
    if (nextResult === "solved") {
      ...
      const improvedBest = current.bestMoves === null ? finalMoves : Math.min(current.bestMoves, finalMoves);
      const nextProgress: DailyPuzzleProgress = {
        ...current,
    ...
    <div className="screen game-screen walnut-live theme-green">
      <div className="wl-top-rail" data-ui="hud">
        ...
        <div className="wl-center-status">
          ...
          <span className="wl-room-code">Attempt {attempts} · Best {bestMoves ?? "--"} · Max {maxMoves}</span>
        </div>
        <div className="wl-player-pill is-you">
          <span className="wl-player-label">Date</span>
          <span className="wl-player-score" style={{ fontSize: "1rem" }}>{puzzle.dateSeed}</span>
        </div>
      </div>
      ...
      <div className="hand-area wl-hand-area" data-ui="tray">
        <div className="tray-rail">
          <div className="tray-center">
            <div className={`hand-container ${match.players.you.hand.length > 7 ? "is-scrollable" : ""}`}>
          ...
          <div className="tray-right" data-ui="actions">
            <div className="tray-controls">
              <button className="btn text compact" onClick={resetPuzzle}>Play Again</button>
        ...
        >
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              ...
              <button className="mode-inline-btn" onClick={resetPuzzle}>Play Again</button>
            </div>
          </div>
        </div>
      )}
