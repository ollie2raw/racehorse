...
**Status: RATIFIED as D-CQ-6 and implemented** on branch `chore/cq9.2-f3-barrel-trim`
(not pushed). Scope shipped = the KEEP/TRIM split below + the two FIX items +
...
| **F3** | 1 / 6 | `client/src/modules/guided/index.ts` | Barrel exports ~40 symbols; only ~6–7 have an importer outside `modules/guided/`. The rest are internal-only. | Trimming the barrel to its real external surface is safe mechanically but `modules/guided/` has 25 imports into protected `learn/` and the barrel is the module's contract — wants a deliberate pass, not a drive-by. **RATIFIED D-CQ-6 (2026-09-08), shipped on `chore/cq9.2-f3-barrel-trim`.** Barrel 57→14 lines (8 kept / 45 trimmed); self-import + dead `modules/match/index.ts` re-export fixed; F5 `why`-comments folded in. `LessonCoachPanelContent` rename excluded from this pass, **resolved separately 2026-09-08** — renamed to `BotGuidedCoachPanelContent`, own PR. Step-1 map `§CQ9.1.7`, graded proposal `§CQ9.2.F3`. Closes #125. |
...
- [ ] REFACTOR / STYLE findings — await per-finding greenlight. (Shipped: F19 `D-CQ-4`; F21/F22/F23 `D-CQ-2`/`D-CQ-3`; batch-1 F13/F17/F9/F20 + F4 + D-CQ-5 via PR #124 (merged `de7256ae`); scrubber F3-adjacent work via PR #129 (merged `b2da1578`); **F3 barrel trim `D-CQ-6`** on `chore/cq9.2-f3-barrel-trim` (F5 `why`-comments folded in). Still held: F6-Tier-3, F12. `LessonCoachPanelContent` name collision (deferred out of D-CQ-6) — **RESOLVED 2026-09-08 (S1, PR #137)**, renamed to `BotGuidedCoachPanelContent`. **F11 — STOP caught a real error in the finding (2 of the 5 `*ClassName` props, `handStackClassName`/`handFooterClassName`, ARE used — `NoBrainerLabScreen.tsx:402–403` passes them through `MatchLiveLayout`), then RESOLVED 2026-09-08 (S2, PR #138)**: the corrected 3-dead-prop scope (`studioShellClassName`, `boardZoneClassName`, `handDockClassName`) trimmed, the 2 live props left untouched.**)
