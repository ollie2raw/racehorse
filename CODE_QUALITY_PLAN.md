...
- [ ] REFACTOR / STYLE findings — await per-finding greenlight. (Shipped: F19 `D-CQ-4`; F21/F22/F23 `D-CQ-2`/`D-CQ-3`; batch-1 F13/F17/F9/F20 + F4 + D-CQ-5 via PR #124 (merged `de7256ae`); scrubber F3-adjacent work via PR #129 (merged `b2da1578`); **F3 barrel trim `D-CQ-6`** on `chore/cq9.2-f3-barrel-trim` (F5 `why`-comments folded in). Still held: F6-Tier-3, F12. `LessonCoachPanelContent` name collision (deferred out of D-CQ-6) — **RESOLVED 2026-09-08**, renamed to `BotGuidedCoachPanelContent`. **F11 — STOP: 2 of the 5 `*ClassName` props (`handStackClassName`, `handFooterClassName`) ARE used — `NoBrainerLabScreen.tsx:402–403` passes them through `MatchLiveLayout`. Finding is wrong as written; not trimmed.**)
