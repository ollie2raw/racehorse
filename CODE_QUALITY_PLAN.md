...
site; the duplicated name is a readability trap. **RESOLVED 2026-09-08 (S1,
`LAUNCH_READINESS_CHECKLIST.md`) — local interface renamed to
`BotGuidedCoachPanelContent`.** Confirmed the two shapes had genuinely
diverged (every field optional locally vs. every field required on the
canonical type) — the divergence is what made a rename the correct fix
rather than a merge onto the canonical type, per this row's own
recommendation. Confirmed no external consumer imported the local type by
name (`tsc -b` clean after the rename is the proof). See §CQ9.4 checklist.
...
### CQ9.2.F3 F3 — Step-2 graded proposal → **ratified D-CQ-6, shipped 2026-09-08**

**Status: RATIFIED as D-CQ-6, implemented, and MERGED** — branch
`chore/cq9.2-f3-barrel-trim` → **PR #131, merged `6c3b5f7a` on 2026-09-08**
(closed #125). Scope shipped = the KEEP/TRIM split below + the two FIX items +
F5's `why`-comments. **Excluded from this pass, resolved separately:** the
`LessonCoachPanelContent` name collision (item 3 below) — noted, not touched
this pass, then fixed as its own scoped rename 2026-09-08 (S1,
`LAUNCH_READINESS_CHECKLIST.md`) — see the resolved note above. Full
verification bar green (see the D-CQ-6 decisions-log row). The proposal text
is kept below as the record of what was graded.
...
3. **Name collision** — `bot/BotGuidedMatchPanel.tsx:5` local
   ...
   priority — safe to defer if it widens the diff. **RESOLVED 2026-09-08** —
   renamed to `BotGuidedCoachPanelContent`, own scoped PR, not folded into
   this pass.
...
| **F3** | 1 / 6 | `client/src/modules/guided/index.ts` | Barrel exports ~40 symbols; only ~6–7 have an importer outside `modules/guided/`. The rest are internal-only. | Trimming the barrel to its real external surface is safe mechanically but `modules/guided/` has 25 imports into protected `learn/` and the barrel is the module's contract — wants a deliberate pass, not a drive-by. **RATIFIED D-CQ-6 (2026-09-08), merged as PR #131 (`6c3b5f7a`) — closed #125.** Barrel 57→14 lines (8 kept / 45 trimmed); self-import + dead `modules/match/index.ts` re-export fixed; F5 `why`-comments folded in. `LessonCoachPanelContent` rename excluded from this pass, **resolved separately 2026-09-08** — renamed to `BotGuidedCoachPanelContent`, own PR #137. Step-1 map `§CQ9.1.7`, graded proposal `§CQ9.2.F3`. **DONE — nothing left in F3 scope.** |
| **F6** | 9 | `client/src/modules/guided/` (whole area) | ~3,516 LOC, **one** vitest file (`useGuidedLessonBoot.test.tsx`). V2 playback, placement handlers, coach-presentation builder, authoring capture — all untested. | The single largest coverage gap in scope, but writing tests for a live lesson runtime coupled to protected `learn/` is a project, not a step. Needs its own scoped effort + greenlight. **DONE — Tiers 1–3 (files 1–8); Tier 3 via PR #158, closed #126. §CQ9.5.4 effect-heavy hooks deferred with reasons.** |
| **F11** | 6 | `client/src/match/board/InGameBoardFrame.tsx` | **Corrected 2026-09-08 (S1 STOP finding verified, then S2 fixed) — the finding as originally written was wrong.** 12 props, 5 of them `*ClassName` escape hatches. Only **3** (`studioShellClassName`, `boardZoneClassName`, `handDockClassName`) were actually dead — its only caller (`MatchLiveLayout`) never passed them. The other 2 (`handStackClassName`, `handFooterClassName`) **are live** — `NoBrainerLabScreen.tsx:402–403` passes them through `MatchLiveLayout`. | **RESOLVED — trimmed to the corrected 3-dead scope** (`LAUNCH_READINESS_CHECKLIST.md` S2). The 2 live props left untouched. Speculative-generality risk on a load-bearing layout component was the original hesitation; verifying against every caller first (not just `MatchLiveLayout`) is what made trimming safe. |
...
- [ ] REFACTOR / STYLE findings — await per-finding greenlight. (Shipped: F19 `D-CQ-4`; F21/F22/F23 `D-CQ-2`/`D-CQ-3`; batch-1 F13/F17/F9/F20 + F4 + D-CQ-5 via PR #124 (merged `de7256ae`); scrubber F3-adjacent work via PR #129 (merged `b2da1578`); **F3 barrel trim `D-CQ-6`** via PR #131 (merged `6c3b5f7a`, closed #125; F5 `why`-comments folded in). **F6 Tier-3 done via PR #158 (`b9c02b2e`, closed #126) — F6 complete.** Still held: F12/F15 (#127/#128). `LessonCoachPanelContent` name collision (deferred out of D-CQ-6) — **RESOLVED 2026-09-08 (S1, PR #137)**, renamed to `BotGuidedCoachPanelContent`. **F11 — STOP caught a real error in the finding (2 of the 5 `*ClassName` props, `handStackClassName`/`handFooterClassName`, ARE used — `NoBrainerLabScreen.tsx:402–403` passes them through `MatchLiveLayout`), then RESOLVED 2026-09-08 (S2, PR #138)**: the corrected 3-dead-prop scope (`studioShellClassName`, `boardZoneClassName`, `handDockClassName`) trimmed, the 2 live props left untouched.**)
...
- [ ] **Held findings filed as tracking issues (2026-09-08)** — F3 → **#125 CLOSED** (D-CQ-6, PR #131), F6 Tier-2/3 → **#126 CLOSED** (PR #158), F12 → **#127**, F15 → **#128** (still open). Parked explicitly, same discipline as #116/#117.
- [x] **F3 — barrel trim shipped as D-CQ-6** (2026-09-08, PR #131, merged `6c3b5f7a`, closed #125). Step-1 map `§CQ9.1.7`; graded proposal `§CQ9.2.F3`; ratification + full verification in the D-CQ-6 decisions-log row. Barrel 57→14; self-import + dead `modules/match` re-export fixed; F5 `why`-comments done. `LessonCoachPanelContent` rename left as open STYLE at the time — **resolved 2026-09-08** as its own scoped rename to `BotGuidedCoachPanelContent` (S1, `LAUNCH_READINESS_CHECKLIST.md`, PR #137).
- [x] **`LessonCoachPanelContent` name collision — RESOLVED 2026-09-08** (S1, `LAUNCH_READINESS_CHECKLIST.md`, merged PR #137). `bot/BotGuidedMatchPanel.tsx`'s local interface renamed to `BotGuidedCoachPanelContent`. Confirmed the two shapes had genuinely diverged (every field optional locally, every field required on the canonical `guidedCoachPresentationTypes.ts` type) before deciding to rename rather than merge — per this finding's own instruction, a real divergence means rename is the safer fix. Confirmed no external consumer imported the local type by name (checked directly, and `tsc -b` clean after the rename is independent proof). Pure rename, zero runtime behaviour change. Verified green: `tsc -b`, client vitest 215/1617, lint 51/51 (0 errors), `check:architecture` 20/20.
- [x] **F11 — RESOLVED 2026-09-08** (S2, `LAUNCH_READINESS_CHECKLIST.md`, merged PR #138). The finding as originally written was wrong (claimed all 5 `*ClassName` props on `InGameBoardFrame` were dead) — the §CQ9.4 STOP note already caught that 2 of the 5 (`handStackClassName`, `handFooterClassName`) are live via `NoBrainerLabScreen.tsx:402–403` → `MatchLiveLayout`. Confirmed on current `main` (re-verified, not assumed stale): `MatchLiveLayout` is still `InGameBoardFrame`'s only caller and still never passes `studioShellClassName`/`boardZoneClassName`/`handDockClassName`. Removed those 3 from `InGameBoardFrameProps`, the destructure, and the 3 template interpolations (each collapsed back to its bare class name); the 2 live props and their interpolations left untouched. `§CQ9.2`'s F11 row rewritten to the corrected finding rather than left contradicting its own STOP note. Verified green: `tsc -b` (a missed consumer would be a compile error — none), client vitest 215/1617, lint 51/51 (0 errors), `check:architecture` 20/20, `check:deps`.
...
- [x] **F6 — guided module test coverage — COMPLETE.** Greenlit 2026-09-05, scoping pass `§CQ9.5`. Files 1–5 (Tiers 1–2) landed as direct commits; **files 6–8 (Tier 3) via PR #158 (`b9c02b2e`), closed #126** — `useGuidedLessonBoot.test.tsx` extended 1→18, new `useGuidedV2CoordinationState.test.ts` (6) + `guidedPlacementHandlers.test.tsx` (11, `handleGuidedPlacement` decision logic only; no helper extraction). Effect-heavy hooks in `§CQ9.5.4` stay deferred with reasons. Full log below.
  ...
  **Fixture-generation findings (the plan flagged real risk here — confirmed):**
  ...
  CERTIFIED.
  **Files 6–8 landed (Tier 3) — PR #158 (`b9c02b2e`), closed #126. Test-only, no source changes.**
  File 6: `useGuidedLessonBoot.test.tsx` extended 1→18 — the `prop && mode === 'bot'`
  gate; `frozenLesson` source (`loadFrozenLesson` → authoring-session fallback →
  none); `guidedTranscript` source (published → draft → suppressed in authoring /
  V2 mode); the `isGuidedTranscriptMode` / `isGuidedFrozenLessonMode` /
  `lessonLayoutMode` truth table incl. ready vs unstartable V2 playback.
  File 7: `useGuidedV2CoordinationState.test.ts` (new) — 6 cases: the lazy-init
  guard chain (not V2 / no lesson / not preloaded / cannot-start each → `0` with
  the registry untouched; happy path → `initGuidedV2Playback(lesson, 1).firstEventIndex`).
  File 8: `guidedPlacementHandlers.test.tsx` (new) — 11 cases, `handleGuidedPlacement`
  decision logic only. **Did NOT do the optional `matchesExpectedGuidedMove`
  extraction** (§CQ9.5.6 item 8's "preferred" path) — it is a source change and
  this pass was scoped pure test-writing; the `renderHook` + spy-deps approach
  covered every branch (transcript no-turn / non-play / off-line / match; V2
  match / tile-mismatch / position-mismatch / already-off-line; neither mode).
  Green: `tsc -b`, client vitest 223/1697, server vitest all shards, lint 49/50,
  `check:architecture` 20/20, `check:bot-match-lazy --dist`.
...
|---|---|---|---|
| **D-CQ-6** | 2026-09-08 | **F3 ratified — `modules/guided/index.ts` barrel trimmed to its real external contract.** Scope: the KEEP/TRIM split from `§CQ9.2.F3` (8 kept: `useAuthoringCapture`, `useGuidedLessonBoot`, `useGuidedMatchRuntime`, `useGuidedMatchCommandEffects`, `useGuidedMatchCaptureRuntime`, `useGuidedV2CoordinationState`, `UseGuidedLessonBootResult`, `GuidedPlacementResult`; 45 internal-only re-exports removed — each still exported from its own file, every internal guided consumer already imports by path) + the two FIX items: the self-import `useGuidedV2CoordinationState.ts:6` (`./index.ts` → `./useGuidedLessonBoot.ts`) and the **dead re-export** `modules/match/index.ts:22–26` (`useAuthoringCapture` + 2 types, zero consumers — deleted, same class as F18) + F5's 8 `exhaustive-deps` `why`-comments in `modules/guided/`. Barrel: 57 lines → 14. **Excluded from this pass (still-open STYLE at the time):** the `bot/BotGuidedMatchPanel.tsx` local `LessonCoachPanelContent` vs the canonical `guidedCoachPresentationTypes.ts` name collision — noted, not touched. **Resolved separately 2026-09-08** — renamed to `BotGuidedCoachPanelContent`, its own scoped PR (S1, `LAUNCH_READINESS_CHECKLIST.md`). | Pure export-surface + one import-path fix + one dead-shim delete. **Zero `learn/` surface** (no `learn/` barrel, no cycle — Step-1 confirmed), **zero runtime behaviour change**. TypeScript turns any missed consumer into a compile error. Verified green: `tsc -b`, client vitest 215/1616, server vitest 218/1300, lint 51/51 (0 errors), `lint:css`, `check:architecture` 20/20, `check:deps` (3407→3388 dep edges), `check:multiplayer-arch`, `check:socket-registry`, **`check:bot-match-lazy` including `--dist` on a fresh build** (per the ENGINEERING_GUARDRAILS.md §8 discipline established by PR #129). Branch `chore/cq9.2-f3-barrel-trim` → **PR #131, merged `6c3b5f7a` 2026-09-08, closed #125.** |
