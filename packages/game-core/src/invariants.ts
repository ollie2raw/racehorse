...
import type { GameState, BoardState, Tile } from './types';
import { generateFullSet, totalTilesInSet } from './types';
...
export function isValidTile(tile: unknown, maxPips = 6): boolean {
  if (!tile || typeof tile !== 'object') return false;
  const candidate = tile as Tile;
  return Number.isInteger(candidate.low)
    && Number.isInteger(candidate.high)
    && candidate.low >= 0
    && candidate.high >= 0
    && candidate.low <= maxPips
    && candidate.high <= maxPips;
}

export function collectPhysicalTilesFromBoard(board: BoardState | null): Tile[] {
  if (!board) return [];
  const tiles = board.mainLine.map((placed) => placed.tile);
  for (const hub of board.hubDoubles) {
    for (const branch of hub.branches) {
      if (!branch) continue;
      tiles.push(...branch.tiles.map((placed) => placed.tile));
    }
  }
  return tiles;
}

export function collectPhysicalTileIds(tiles: readonly Tile[]): string[] {
  return tiles.map(tileKey);
}

export function assertUniquePhysicalTiles(tiles: readonly Tile[], context = 'tiles'): void {
  const seen = new Set<string>();
  for (const tile of tiles) {
    const key = tileKey(tile);
    if (seen.has(key)) throw new Error(`[invariant:${context}] duplicate physical tile ${key}`);
    seen.add(key);
  }
}

export function assertTileIsInRuleset(tile: Tile, maxPips = 6, context = 'tile'): void {
  if (!isValidTile(tile, maxPips)) throw new Error(`[invariant:${context}] invalid tile ${tileKey(tile)}`);
  const exists = generateFullSet(maxPips).some((candidate) => tileKey(candidate) === tileKey(tile));
  if (!exists) throw new Error(`[invariant:${context}] tile is outside the active ruleset`);
}
...
export function tilesOnBoard(board: BoardState | null): Tile[] {
  return collectPhysicalTilesFromBoard(board);
}
