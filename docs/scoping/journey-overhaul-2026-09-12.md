...
- **Content bug found by PR 4's engine-verification tool**: `ch1-n03` ("Open Ends Trial"), the one production puzzle with `boardState` populated, claims its board's open ends are 3 and 5 and that the correct answer is the 3-4 tile. The actual tile chain (`[6,6]-[6,3]-[5,3]-[5,5]`) has both ends capped by doubles, so its genuine open ends are 6 and 5 — the authored `correctTile` (3-4) is not a legal move against this board at all. The only hand tile that actually is legal is 5-6, the tile the puzzle's own narrative frames as the impulsive *wrong* answer. This was never caught because `InteractivePuzzleModal`'s answer check is a plain tile-equality comparison against `correctTile`, not a real engine legality check. Confirmed via `client/src/journey/journeyPuzzleEngineVerification.test.ts`. Not fixed here — it's a `journeyPuzzles.ts` content edit, out of scope for PR 4 (tooling-only) — flagged for a content-only follow-up, ideally alongside PR 6.
- `puzzleBoardAdapter.ts`'s `puzzleToBoardState` sets `leftEndIsDouble: ends[0] === ends[0]`, a self-comparison that is always `true` regardless of the actual value — almost certainly a typo for an actual is-double check. Found while building PR 4's engine-verification tool; did not turn out to be the cause of the `ch1-n03` bug above (that's a genuine authoring error independent of this), and doesn't appear to visibly affect the puzzle board's static rendering, but it's a real, confirmable defect in shared display code worth fixing opportunistically rather than carrying forward — not fixed here because it's outside PR 4's tooling-only scope and touching shared display code without a concrete visible bug to fix against carries needless regression risk for this PR.
