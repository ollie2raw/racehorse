...
|---|---|---|---|---|
| 18 | **D2 — `Board.tsx` decomposition** | `REFACTOR` §3 | large | **DONE 2026-09-10.** Scoping: `D2-board-tsx-decomposition-scoping.md`. Tier-0 geometry move (#173), pre-req `board-camera.spec.ts` (#176 — confirmed `fritz-play-to-completion.spec.ts` covered placement but not camera-fit; mutation-tested, loop-tested 10×), then the three hooks: `useBoardRenderLayout` (#177), `useBoardCamera` (#178 — implemented before `useBoardPointerControls`, reversing the doc's PR-3/PR-4 order once the real dependency direction surfaced), `useBoardPointerControls` (#179). `Board.tsx`: 1266 → 396 lines. `board-camera.spec.ts` loop-tested 10/10 clean against the complete decomposition. |
...
| 20 | **D5 — `App.tsx`** | `REFACTOR` §3 | large (incremental parts cheap) | Scoping: `D5-...md`. §6 already found PRs 1–4 don't exist as safe chips. 2026-09-10: **PR 5 also a no-go as scoped.** The doc missed `client/src/multiplayer/runtime/` — `createMultiplayerRuntime()` already consolidates the ~25 refs into one `MultiplayerRuntime` object, and `runtimeSelectors.ts`'s `selectLegacyAppSessionRuntime` (comment: "Legacy … selectors only, no construction") shows an in-progress internal migration the doc never examined. What's actually left in App.tsx is ~25 raw `useRef`s each read 2–6× directly by 6+ separate hook calls (`useMultiplayerResync`, `useMultiplayerRoomCallbacks`, `useMultiplayerConnectionHostParams`, three `useRegister*SocketHandlers`) — none single-consumer, no free corner to peel off. A `useMultiplayerConnectionLifecycle` extraction is either pure churn (wrap the refs, still return ~20 of them to feed the other hooks unchanged) or the full ~1-week all-or-nothing move the doc always flagged as the risk. **No small-reviable-commit path exists for PR-5 as scoped.** The real next step was different work — finishing that migration (consumer hooks reading the runtime object instead of raw refs), scoped separately in `D5-runtime-migration-scoping-2026-09-10.md`. **DONE 2026-09-11.** Register-handlers batch needed no PR (investigated, don't touch the bootstrap — a scoping correction reported before proceeding). `useMultiplayerResync` (#183) and `useMultiplayerRoomCallbacks` (#185) both converted to read `runtime: MultiplayerRuntime` slices instead of individual bootstrap refs; confirmed orphan refs (no runtime slice today: `maxEventSequenceRef`, `roomMatchIdRef`, `roomOperationEpochRef`, `shellDelegatesRef`, `appModeRef`, `applyRoomEventMetaRef`, `schedulePlayerReadyRef`, `trySchedulePlayerReadyRef`) stayed individual params rather than forcing new protected-module additions. Dead `useAppSessionRuntime()` deleted (#184). Each PR: manual ref-identity test, `check:architecture` CERTIFIED 20/20, full MP e2e (11/11) + soak gate. |
...
| 22 | **FC-DEAD-1** — ~2k LOC of unmounted Fritz-Challenge server code. | `LAUNCH_READINESS` §3.6 / `HARDENING_PLAN` "sixth pool" | medium | **CODE DELETION DONE 2026-09-10.** Proof-of-no-dependency pass confirmed the whole cluster (`fritzChallenge.ts`, `fritzChallenges.ts` route + tests, `fritzChallengeStore.ts`/`fritzChallengeCommandStore.ts` + tests) was importer-only-of-itself; deleted (~2.3k LOC) plus the orphaned `soak:fritz-challenge-authority` script and the moot Guardrail #4 `requireStateDigests` residual with it. Full server bar green (220/220 files, 1327/1327 tests). **`fritz_challenge*` tables/migrations intentionally untouched** — dropping them needs its own migration, flagged separately, not bundled into this PR. |
...
| 25 | **End-to-end "play to completion" coverage** for Play vs Fritz, Puzzle Rush, Tournament, Ghost, The Lab. Drivers are ~60 lines against stable selectors (proven this session); Ghost + The Lab had *zero* functional coverage. | `FEATURE_COMPLETENESS` §5.1 | medium–large | **Fritz, Puzzle Rush, Ghost, The Lab: DONE 2026-09-10/11** (#180 Puzzle Rush, #181 The Lab, plus the earlier Fritz spec and `ghost-play-to-completion.spec.ts`, #186). **Tournament: CLOSED 2026-09-11, no build.** Investigated whether `scheduled_start` has any test/dev override in the seed RPC or cron path — none exists; a Tournament completion run is a real 4-account fixture against the actual scheduling wait, 30–45 min wall-clock dominated by idle time until `scheduled_start` fires, not engineering effort. That cost needs a product sign-off on the fixture spend, not an engineering call, and no override exists to shortcut it. **No further action without a separate product decision to add a test-mode override** for `scheduled_start` (or an explicit sign-off to eat the 30–45 min real-time cost per run). |
