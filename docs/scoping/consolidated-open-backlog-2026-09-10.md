...
|---|---|---|---|---|
| 18 | **D2 — `Board.tsx` decomposition** | `REFACTOR` §3 | large | Scoping: `D2-board-tsx-decomposition-scoping.md`. Tier-0 geometry move is a free R-class win; the real hook extraction needs board-interaction e2e first (doesn't exist). |
| 19 | **D4 — `rooms.ts` (+ `generatePuzzles.ts`)** | `REFACTOR` §3 | large | Scoping: `D4-...md`. `rooms.ts` = MP spine, high blast radius, no CI ratchet, `HARDENING` System 2 just certified it as-is. `generatePuzzles.ts` = low-value tidy, defer. |
| 20 | **D5 — `App.tsx`** | `REFACTOR` §3 | large (incremental parts cheap) | Scoping: `D5-...md`. PRs 1–4 (bundle grouping, nav-sync hook) are safe S8-shaped chips worth doing anytime; PR 5 (multiplayer-connection-lifecycle hook) is the real project. **Highest value-per-risk of the three D-items.** |
