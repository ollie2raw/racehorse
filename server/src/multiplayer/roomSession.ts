...
export function scheduleRoomCleanup(roomCode: string): void {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      log.error(error, '[room-match-logs] failed to archive room before cleanup:');
    }
...
export function scheduleBotTurn(roomCode: string): void {
  ...
  const timer = setTimeout(async () => {
    ...
    try {
      ...
      log.warn(error, '[bot] turn failed');
    }
  }, delayMs);
  ...
  for (let hubIdx = 0; hubIdx < board.hubDoubles.length; hubIdx++) {
    ...
    hub.branches.forEach((arm: BranchArm | undefined | null, armIdx: number) => {
      if (!arm || typeof arm !== 'object' || !Array.isArray(arm.tiles)) {
        log.warn(`[maskStateForRecipient:${ctx}] hub ${hubIdx} arm ${armIdx} missing BranchArm.tiles array`);
      }
    });
  }
}
            ...
            log.info(
              {
                connectionId,
                recipientPlayerId,
                picks: room.preGameDraw?.picks,
                youWillBe: room.preGameDraw?.picks[recipientPlayerId ?? ''] ?? null,
              },
              'broadcasting pregame draw to socket',
            );
      ...
      if (pid.startsWith('bot:fritz:') && !roomAfter.nextHandReady.has(pid)) {
        setTimeout(async () => {
          try {
            ...
            log.warn(e, '[bot] auto-ready error');
          }
        }, 800);
      }
    }
