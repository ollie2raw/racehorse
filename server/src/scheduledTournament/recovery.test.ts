...
describe('recoverTournamentMatches', () => {
  ...
  it('recreates a missing in-progress room and re-dispatches ready matches', async () => {
    ...
    const persistence: EnginePersistence = {
      ...
      completeTournamentMatch: vi.fn(async () => { throw new Error('not exercised by recovery.test'); }),
      promoteTournamentMatch: vi.fn(async () => { throw new Error('not exercised by recovery.test'); }),
